  ...
  function runSlideshowVideo(imgs, per = 2600) {
    {
      ...
      try {
        ...
        const sndCtx = soundClipInto(stream, dur / 1000);
        let rec;
        ...
        rec.onstop = () => {
          if (sndCtx) setTimeout(() => { try { sndCtx.close(); } catch (e) {} }, 300);
...
STRICT: no watermark, no signature, no modern objects${withText ? ", no text other than the two captions specified above" : ", no text or lettering of any kind"}.`;
  ...
  // bake into a MediaRecorder stream: returns the AudioContext to close on stop
  function soundClipInto(stream, durSecs) {
    ...
    try {
      ...
      buildSablesound(actx, dest, palName, durSecs, actx.currentTime + 0.05);
