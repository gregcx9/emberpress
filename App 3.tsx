  ...
  function runSlideshowVideo(imgs, per = 2600) {
    {
      ...
      try {
        ...
        let rec;
        try { rec = new MediaRecorder(recStream, { mimeType: "video/mp4", videoBitsPerSecond: 8_000_000, audioBitsPerSecond: 128_000 }); }
        catch { rec = new MediaRecorder(recStream); }
        const chunks = [];
        rec.ondataavailable = (e) => e.data.size && chunks.push(e.data);
        rec.onstop = () => {
          if (snd) { try { snd.track.stop(); } catch (e) {} setTimeout(() => { try { snd.actx.close(); } catch (e) {} }, 300); }
...
STRICT: no watermark, no signature, no modern objects${withText ? ", no text other than the two captions specified above" : ", no text or lettering of any kind"}.`;
  ...
  function soundClipInto(videoStream, durSecs) {
    ...
    try {
      const actx = new AC();
      if (actx.state === "suspended") actx.resume(); // iOS: fresh contexts start suspended → silent track
      const mediaDest = actx.createMediaStreamDestination();
      // bus feeds BOTH the recorder track and an audible monitor —
      // hearing it during recording confirms the sound is generating
      ...
      if (!track) { try { actx.close(); } catch (e) {} return null; }
      // Safari is unreliable with addTrack on canvas capture streams —
      // build one combined stream instead
  ...
  function soundAloud(durSecs, delaySecs = 0) {
    ...
    try {
      ...
      buildSablesound(actx, actx.destination, palName, durSecs, actx.currentTime + delaySecs);
    } catch (e) { /* silent */ }
  }
