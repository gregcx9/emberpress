...
// ============================================================
// SABLESOUND — original dark-drone soundtrack baked into every
// recorded video. Palette-matched. Riser 0–2.5s, impact hit at
// 2.5s (cut your hook text to it), φ-stacked drone body, clean
// fade tail. Registers on TikTok as your original sound.
// ============================================================
const PHI_S = 1.6180339887498949;
const SOUND_RATIOS = [1 / PHI_S, 1, PHI_S, PHI_S * PHI_S];
const SOUND_PRESETS = {
  "Ember (—BORN core)": { root: 111, el: { drone: 1, heartbeat: 1, riser: 1, hit: 1, shimmer: 0 } },
  "Tide (TIDEBORN)": { root: 222, el: { drone: 1, heartbeat: 0, riser: 1, hit: 0, shimmer: 1 } },
  "Frost (COLDBORN)": { root: 174, el: { drone: 1, heartbeat: 0, riser: 1, hit: 0, shimmer: 1 } },
  "Blood (BLOODBORN)": { root: 111, el: { drone: 1, heartbeat: 1, riser: 1, hit: 1, shimmer: 0 } },
  "Ash (ASHBORN)": { root: 174, el: { drone: 1, heartbeat: 1, riser: 1, hit: 1, shimmer: 0 } },
  "Gilt (GIFTBORN)": { root: 222, el: { drone: 1, heartbeat: 0, riser: 1, hit: 1, shimmer: 1 } },
  "Toll (TOLLBORN)": { root: 174, el: { drone: 1, heartbeat: 0, riser: 1, hit: 1, shimmer: 0 } },
};
function buildSablesound(actx, dest, palName, durSecs, t0 = 0) {
  const preset = SOUND_PRESETS[palName] || SOUND_PRESETS["Ember (—BORN core)"];
  const { root, el } = preset;
  const intensity = 0.8;
  const dur = Math.max(3, durSecs);
  const hitTime = t0 + Math.min(2.5, dur * 0.35);
  const end = t0 + dur;
  const bodyEnd = end - 1.2;
  ...
  // φ drone — faint under the riser, swells after the hit
  if (el.drone) {
    const lfo = actx.createOscillator();
    lfo.frequency.value = PHI_S / 10;
    ...
    filt.connect(master);
    SOUND_RATIOS.forEach((r, i) => {
      const o = actx.createOscillator();
      o.type = "sine";
      o.frequency.value = root * r;
      o.detune.value = (i - 1.5) * 0.618;
      const g = actx.createGain();
      const peak = [0.26, 0.34, 0.18, 0.07][i] * intensity;
      g.gain.setValueAtTime(0.0001, t0);
      g.gain.exponentialRampToValueAtTime(peak * 0.25, hitTime);
      g.gain.exponentialRampToValueAtTime(peak, hitTime + 1.6);
      g.gain.setValueAtTime(peak, Math.max(hitTime + 1.7, bodyEnd));
      g.gain.linearRampToValueAtTime(0.0001, end - 0.05);
      lfoG.connect(g.gain);
      const p = actx.createStereoPanner ? actx.createStereoPanner() : null;
      if (p) { p.pan.value = [0, 0, -0.3, 0.3][i]; o.connect(g); g.connect(p); p.connect(filt); }
      else { o.connect(g); g.connect(filt); }
      o.start(t0); o.stop(end);
    });
  }

  // heartbeat — lub-dub at ~64 bpm through the body
  if (el.heartbeat) {
    const beat = 60 / 64;
    for (let t = hitTime + 0.4; t < bodyEnd; t += beat) {
      [[0, 1.0], [0.22, 0.6]].forEach(([off, vel]) => {
        const o = actx.createOscillator();
        o.type = "sine";
        o.frequency.setValueAtTime(58, t + off);
        o.frequency.exponentialRampToValueAtTime(38, t + off + 0.12);
        const g = actx.createGain();
        const pk = 0.5 * vel * intensity;
        g.gain.setValueAtTime(0.0001, t + off);
        g.gain.exponentialRampToValueAtTime(pk, t + off + 0.012);
  ...
  // riser — filtered noise sweep into the hit
  if (el.riser) {
    ...
    bp.frequency.exponentialRampToValueAtTime(2200, hitTime);
    const g = actx.createGain();
    g.gain.setValueAtTime(0.0001, t0);
    g.gain.exponentialRampToValueAtTime(0.22 * intensity, hitTime - 0.05);
    g.gain.exponentialRampToValueAtTime(0.0001, hitTime + 0.15);
    src.connect(bp); bp.connect(g); g.connect(master);
    src.start(t0); src.stop(hitTime + 0.3);
  }

  // impact hit — pitch-dropped boom + dark burst
  if (el.hit) {
    const boom = actx.createOscillator();
    boom.type = "sine";
    boom.frequency.setValueAtTime(95, hitTime);
    boom.frequency.exponentialRampToValueAtTime(34, hitTime + 0.5);
    const bg = actx.createGain();
    bg.gain.setValueAtTime(0.0001, hitTime);
    bg.gain.exponentialRampToValueAtTime(0.9 * intensity, hitTime + 0.015);
    bg.gain.exponentialRampToValueAtTime(0.0001, hitTime + 1.6);
    boom.connect(bg); bg.connect(master);
    boom.start(hitTime); boom.stop(hitTime + 1.8);
    const burst = actx.createBufferSource();
    burst.buffer = noise;
    const lp = actx.createBiquadFilter();
    lp.type = "lowpass";
    lp.frequency.setValueAtTime(3000, hitTime);
    lp.frequency.exponentialRampToValueAtTime(180, hitTime + 0.5);
    const ng = actx.createGain();
    ng.gain.setValueAtTime(0.0001, hitTime);
    ng.gain.exponentialRampToValueAtTime(0.35 * intensity, hitTime + 0.01);
    ng.gain.exponentialRampToValueAtTime(0.0001, hitTime + 0.6);
    burst.connect(lp); lp.connect(ng); ng.connect(master);
    burst.start(hitTime); burst.stop(hitTime + 0.8);
  }

  // gold shimmer — high φ³ partial with slow tremolo
  if (el.shimmer) {
    const o = actx.createOscillator();
    o.type = "sine";
    o.frequency.value = root * PHI_S * PHI_S * PHI_S;
    ...
    trem.connect(tg);
    const g = actx.createGain();
    const pk = 0.045 * intensity;
    g.gain.setValueAtTime(0.0001, t0);
    g.gain.exponentialRampToValueAtTime(pk, hitTime + 2);
    g.gain.setValueAtTime(pk, Math.max(hitTime + 2.1, bodyEnd));
    g.gain.linearRampToValueAtTime(0.0001, end - 0.05);
    tg.connect(g.gain);
    o.connect(g); g.connect(master);
    o.start(t0); o.stop(end);
    trem.start(t0); trem.stop(end);
  }
}
  ...
  function runSlideshowVideo(imgs, per = 2600) {
    {
      ...
      const drawFrame = (el) => {
        ...
        // Ken Burns v9: eased travel, alternating in/out per slide. Text-safe
        // mode caps the zoom at 1.08 with no drift — max crop ≈3.5% per edge,
        // so hooks, handles and attributions baked into cards stay in frame.
        // Full mode keeps v8's 18% travel for imagery without text.
        const ease = (p) => (p <= 0 ? 0 : p >= 1 ? 1 : p * p * (3 - 2 * p));
        const drawImg = (im, alpha, zoomP, slideIdx) => {
          const p = ease(zoomP);
          const zoomIn = slideIdx % 2 === 0;
          const z = textSafe
            ? (zoomIn ? 1.005 + 0.075 * p : 1.08 - 0.075 * p)
      ...
      if (!canMp4) {
        ...
        setRecording(true);
        const pre = 3000;
        soundAloud(dur / 1000, pre / 1000); // screen recording captures device audio
      ...
      try {
        ...
        const snd = soundClipInto(stream, dur / 1000);
        const recStream = snd ? snd.combined : stream;
        const rec = makeRecorder(recStream);
        reportRecording(rec, recStream);
        const chunks = [];
        rec.ondataavailable = (e) => e.data.size && chunks.push(e.data);
        rec.onstop = () => {
          if (snd) { try { snd.track.stop(); } catch (e) {} }
...
STRICT: no watermark, no signature, no modern objects${withText ? ", no text other than the two captions specified above" : ", no text or lettering of any kind"}.`;
  ...
  function getSoundCtx() {
    ...
    if (!AC) return null;
    if (!soundCtxRef.current || soundCtxRef.current.state === "closed") soundCtxRef.current = new AC();
    const actx = soundCtxRef.current;
    if (actx.state === "suspended") actx.resume();
    return actx;
  }
  // WebKit mutes pure Web Audio when the iPhone silent switch is on,
  // but <audio>-element playback ignores the switch — so all audible
  // output is routed through a hidden <audio> element.
  ...
  function playThroughElement(actx, stream, durSecs) {
  ...
  // 5-second audible test of the current palette's sound
  function testSound() {
    const actx = getSoundCtx();
    if (!actx) { setSoundStatus("No Web Audio in this browser."); return; }
    const dest = actx.createMediaStreamDestination();
    buildSablesound(actx, dest, palName, 5, actx.currentTime + 0.05);
    playThroughElement(actx, dest.stream, 5);
    setSoundStatus(`Playing 5s test (${palName}) — if silent, raise volume; the silent switch is already bypassed.`);
    setTimeout(() => setSoundStatus(""), 7000);
  }
  // bake into a MediaRecorder stream: returns {actx, combined, track} or null
  function soundClipInto(videoStream, durSecs) {
    if (!soundOn) return null;
    const actx = getSoundCtx();
    if (!actx) return null;
    try {
      const mediaDest = actx.createMediaStreamDestination();
      const monitorDest = actx.createMediaStreamDestination();
      const bus = actx.createGain();
      bus.gain.value = 1;
      bus.connect(mediaDest);
      const mon = actx.createGain();
      mon.gain.value = 0.35;
      bus.connect(mon);
      mon.connect(monitorDest);
      buildSablesound(actx, bus, palName, durSecs, actx.currentTime + 0.1);
      playThroughElement(actx, monitorDest.stream, durSecs); // hear it while it records
      const track = mediaDest.stream.getAudioTracks()[0];
      if (!track) return null;
      const combined = new MediaStream([...videoStream.getVideoTracks(), track]);
      return { actx, combined, track };
    } catch (e) { return null; }
  }
  ...
  // play out loud (for the iOS screen-recording fallback, which captures device audio)
  function soundAloud(durSecs, delaySecs = 0) {
    if (!soundOn) return;
    const actx = getSoundCtx();
    if (!actx) return;
    try {
      const dest = actx.createMediaStreamDestination();
      buildSablesound(actx, dest, palName, durSecs, actx.currentTime + delaySecs);
      playThroughElement(actx, dest.stream, durSecs + delaySecs);
    } catch (e) { /* silent */ }
  }
  ...
  function recordVideoWith(frame = videoFrame, durMs) {
    const c = canvasRef.current, ctx = c.getContext("2d");
  ...
  function recordMotionVideo() {
    ...
    recordVideoWith(motionVideoFrame, motionDurMs());
  }
  ...
  return (
    <div style={S.app}>
      ...
      <header style={S.header}>
        ...
        <span style={S.sub}>2D generator · cards · backdrops · video · motion · v8</span>
      </header>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <div style={S.panel}>
              ...
              <label style={{ ...S.small, display: "flex", alignItems: "center", gap: 6, marginTop: 6 }}>
                <input type="checkbox" checked={soundOn} onChange={(e) => setSoundOn(e.target.checked)} /> 🔊 Bake Sablesound into the video — palette-matched dark drone with an impact hit at 2.5s. Applies to all video exports (main, slideshow, Motion). In screen-recording fallback it plays out loud so the recording captures it.
              </label>
