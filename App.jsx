...
export default function App() {
  ...
  function buildSablesound() {
    ...
    // drone: root (two detuned voices) + fifth, each swelling on its own slow LFO
    const root = pal.accent === "#8FD0D8" ? 49 : pal.accent === "#9FB8D8" ? 55 : 62;
    [[root, "triangle", 0.09], [root + 0.7, "triangle", 0.08], [root * 1.5, "sine", 0.05]].forEach(([f, type, vol], i) => {
    ...
    pulse.connect(pg); pg.connect(master); pulse.start();
    const beat = setInterval(() => {
      const t = ac.currentTime;
      pg.gain.cancelScheduledValues(t);
      pg.gain.setValueAtTime(0.0001, t);
      pg.gain.exponentialRampToValueAtTime(0.06, t + 0.05);
  ...
  function withSound(vStream) {
    if (!soundOn) return { stream: vStream, snd: null };
    try {
      const snd = buildSablesound();
  ...
  function runSlideshowVideo(imgs, per = 2600) {
    {
      ...
      try {
        const { stream, snd } = withSound(c.captureStream(30));
        let rec;
  ...
  return (
    <div style={S.app}>
      ...
      <header style={S.header}>
        ...
        <span style={S.sub}>2D generator · cards · backdrops · video · motion · sound · v9</span>
      </header>
