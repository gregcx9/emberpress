...
export default function App() {
  ...
  function recordSlideshow() {
    ...
    gallery.forEach((g) => {
      const im = new Image();
      im.onload = () => { loaded++; if (loaded === gallery.length) start(); };
      im.src = g.src;
      imgs.push(im);
    });
    function start() {
      const c = canvasRef.current, ctx = c.getContext("2d");
      const per = 2600, fadeMs = 450;
...
#booktok #darkromantasy #fantasyromance #kindleunlimited #kubooks${tag ? " #" + tag : ""} #bookquotes`;
  ...
  function buildPaintingPrompt() {
    ...
    const q1 = `“${v1.replace(/^[“"]|[”"]$/g, "")}”`;
    const q2 = `“${v2.replace(/^[“"]|[”"]$/g, "")}”`;
    const textBlock = paintText
...
No other text anywhere in the image. Double-check both captions letter-for-letter against the text above before finishing.`
    ...
    const p = `Create a 2D image: a traditional oil painting on textured canvas with visible brushstrokes, in a muted, painterly historical-fantasy style. Palette: ${mood}.
...
STRICT: no watermark, no signature, no modern objects${paintText ? ", no text other than the two captions specified above" : ", no text or lettering of any kind"}.`;
    setBuiltPrompt(p);
    robustCopy(p);
  }
  ...
  function offlinePairs(n = 5) {
    const bank = BANKS[palName] || BANKS["Ember (—BORN core)"];
    const rnd = mulberry32((Date.now() + 7) % 100000);
  ...
  return (
    <div style={S.app}>
      ...
      <header style={S.header}>
        ...
        <span style={S.sub}>2D generator · cards · painted backdrops · video</span>
      </header>
