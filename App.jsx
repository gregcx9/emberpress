...
export default function App() {
  ...
  const [voiceSpeed, setVoiceSpeed] = useState(0.8); // 0.5–1.2× — slow-by-default for TTS
  ...
  const resetDefaults = () => {
    ...
    setVoiceMode("unison"); setVoiceVol(1.2); setPulseDepth(0.35); setVoiceSpeed(0.8);
  };
    ...
    <div style={S.page}>
      ...
      <section style={S.panel}>
        ...
        <div style={S.btnRow}>
          ...
          <input ref={fileInputRef} type="file" accept="audio/*,.caf,.m4a,.aac,.mp3,.wav,.aiff,*/*" onChange={onUpload} style={{ display: "none" }} />
        </div>
        ...
        {voiceReady && (
          <>
            ...
            <Row label="Voice speed" val={`${Math.round(voiceSpeed * 100)}%`} />
            <input type="range" min={50} max={120} value={Math.round(voiceSpeed * 100)} onChange={(e) => setVoiceSpeed(Number(e.target.value) / 100)}
              style={{ ...S.range, accentColor: "#E8A34C" }} aria-label="Voice speed" />
            {voiceMode === "unison" && (
              <>
