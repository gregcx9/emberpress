...
export default function App() {
  ...
  // dialogue overlay: call & reply with a timed reveal — the reply is the payoff,
  // so it lands late. Upload CLEAN art (no baked-in text) when using this.
  const [motionDlg1, setMotionDlg1] = useState("");
  const [motionDlg2, setMotionDlg2] = useState("");
  const [motionDlg2At, setMotionDlg2At] = useState(4.5); // seconds — when the reply appears
  // end card: 3s branded close so every view lands on a destination
  const [endCardOn, setEndCardOn] = useState(true);
  const [endTitle, setEndTitle] = useState("THE GIFT HELD");
  const [endLine, setEndLine] = useState("Free to read on Kindle Unlimited");
  ...
  // WebKit mutes raw Web Audio when the iPhone silent switch is on, but
  // <audio>-element playback ignores the switch — all audible output goes
  // through a hidden element.
  function ensureSoundEl() {
    if (!soundElRef.current) {
      const el = document.createElement("audio");
      el.setAttribute("playsinline", "");
      document.body.appendChild(el);
      soundElRef.current = el;
    }
    return soundElRef.current;
  }
  function playThroughElement(stream, durSecs) {
    const el = ensureSoundEl();
    el.srcObject = stream;
    el.volume = 1;
    const p = el.play();
    if (p && p.catch) p.catch(() => {});
    setTimeout(() => { try { el.pause(); el.srcObject = null; } catch (e) {} }, (durSecs + 0.8) * 1000);
  }
  // 5s audible test of the current palette's Sablesound (open hit + late hit)
  function testSound() {
    try {
      const snd = buildSablesound([0.5, 3.2]);
      playThroughElement(snd.monitorStream || snd.stream, 5);
      setTimeout(() => snd.stop(), 5500);
      setSoundStatus("Playing 5s test — if silent, raise volume with the side buttons.");
      setTimeout(() => setSoundStatus(""), 7000);
    } catch (e) { setSoundStatus("No Web Audio in this browser."); }
  }
  // recorder with an mp4+aac codec preference chain (Safari is picky)
    ...
    for (const t of types) {
      try {
        if (MediaRecorder.isTypeSupported && MediaRecorder.isTypeSupported(t)) {
          return new MediaRecorder(stream, { mimeType: t, videoBitsPerSecond: 8_000_000, audioBitsPerSecond: 128_000 });
        }
      } catch (e) { /* try next */ }
    }
    return new MediaRecorder(stream);
  }
  function reportRecording(rec, recStream) {
    setRecInfo(`Last recording — audio track in stream: ${recStream.getAudioTracks().length ? "YES" : "NO"} · recorder type: ${rec.mimeType || "(browser default)"}`);
  }
  // audible playback for the iOS screen-recording fallback (captures device audio)
  function soundAloudFallback(durSecs, delaySecs) {
    if (!soundOn) return null;
    try {
      const cues = [delaySecs + 0.2, delaySecs + Math.max(1, durSecs - 0.5)];
      const snd = buildSablesound(cues);
      playThroughElement(snd.monitorStream || snd.stream, durSecs + delaySecs);
      setTimeout(() => snd.stop(), (durSecs + delaySecs + 1) * 1000);
      return snd;
    } catch (e) { return null; }
  }
  ...
  function buildSablesound(cues = [0.2]) {
    ...
    const ac = new AC();
    if (ac.state === "suspended") ac.resume(); // iOS: fresh contexts start suspended → silent track
    const dest = ac.createMediaStreamDestination();
    const monitorDest = ac.createMediaStreamDestination(); // audible confirmation while recording
    const master = ac.createGain();
    master.gain.value = 0.7;
    // soft limiter + −1 dB ceiling: stacked impacts were peaking at exactly 0 dBFS,
    // which platform re-compression (TikTok/IG) can turn into crackle
    const limiter = ac.createDynamicsCompressor();
    limiter.threshold.value = -6; limiter.knee.value = 4; limiter.ratio.value = 12;
    limiter.attack.value = 0.003; limiter.release.value = 0.25;
    const ceiling = ac.createGain();
    ceiling.gain.value = 0.89; // ≈ −1 dBFS
    master.connect(limiter); limiter.connect(ceiling); ceiling.connect(dest);
    const mon = ac.createGain();
    mon.gain.value = 0.35;
    ceiling.connect(mon);
    mon.connect(monitorDest);
    ...
    return {
      ...
      stop: () => { clearInterval(beat); try { ac.close(); } catch {} try { if (soundElRef.current) { soundElRef.current.pause(); soundElRef.current.srcObject = null; } } catch {} },
    };
  }
  // combine canvas video + Sablesound audio into one recordable stream
  function withSound(vStream, cues, durSecs) {
    if (!soundOn) return { stream: vStream, snd: null };
    try {
      const snd = buildSablesound(cues);
      if (durSecs) playThroughElement(snd.monitorStream, durSecs); // hear it while it records
  ...
  function runSlideshowVideo(imgs, per = 2600) {
    {
      ...
      try {
        ...
        const { stream, snd } = withSound(c.captureStream(30), cutCues, dur / 1000);
        const rec = makeRecorder(stream);
        reportRecording(rec, stream);
        const chunks = [];
...
STRICT: no watermark, no signature, no modern objects${withText ? ", no text other than the two captions specified above" : ", no text or lettering of any kind"}.`;
  ...
  const END_CARD_MS = 3000; // ms branded close appended to motion sequences
  ...
  // dialogue overlay: her line top-left at ~1s, his reply top-right at motionDlg2At.
  // Runs on the GLOBAL clock so the reveal spans cuts; fades out into the end card.
  function drawMotionDialog(ctx, w, h, t, stillsSecs) {
    const l1 = motionDlg1.trim(), l2 = motionDlg2.trim();
    if (!l1 && !l2) return;
    const out = endCardOn ? Math.max(0, Math.min(1, ((stillsSecs - 0.45) - t) / 0.45)) : 1;
    if (out <= 0) return;
    ...
    ctx.textAlign = "center";
    const lh = size * 1.32;
    const drawCol = (text, cx, alpha) => {
      if (!text || alpha <= 0) return;
      ctx.globalAlpha = alpha * out;
      const lines = wrapText(ctx, q(text), w * 0.4);
      let y = h * 0.075 + size;
    ...
    const a1 = Math.max(0, Math.min(1, (t - 1.0) / 0.8));
    const a2 = Math.max(0, Math.min(1, (t - motionDlg2At) / 0.8));
    drawCol(l1, w * 0.27, a1);
    drawCol(l2, w * 0.73, a2);
    ctx.restore();
  }

  // end card: palette-matched branded close — hook, title, attribution, CTA.
  // Ember/grain/vignette layers keep running so it still reads as footage.
  function drawMotionEndCard(ctx, w, h, t, localMs) {
    ctx.fillStyle = pal.bg;
    ctx.fillRect(0, 0, w, h);
    const a = Math.min(1, localMs / 600);
    ctx.save();
    ctx.textAlign = "center";
    const hookTxt = (hook || "").toUpperCase().trim();
    if (hookTxt) {
      ctx.globalAlpha = a;
      ctx.font = `600 ${Math.round(w * 0.03)}px 'Inter', sans-serif`;
      try { ctx.letterSpacing = `${Math.round(w * 0.008)}px`; } catch (e) { /* older WebKit */ }
      ctx.fillStyle = pal.accent;
      ctx.fillText(hookTxt, w / 2, h * 0.36);
      try { ctx.letterSpacing = "0px"; } catch (e) { /* older WebKit */ }
    }
    ctx.globalAlpha = a;
    ctx.font = `700 ${Math.round(w * 0.085)}px 'Spectral', serif`;
    ctx.fillStyle = pal.accent;
    const tl = wrapText(ctx, (endTitle || "").toUpperCase(), w * 0.86);
    let y = h * 0.445;
    tl.forEach((ln) => { ctx.fillText(ln, w / 2, y); y += w * 0.105; });
    ctx.globalAlpha = a * 0.55;
    ctx.strokeStyle = pal.accent; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(w * 0.4, y - w * 0.02); ctx.lineTo(w * 0.6, y - w * 0.02); ctx.stroke();
    ctx.globalAlpha = a;
    ctx.font = `600 ${Math.round(w * 0.034)}px 'Inter', sans-serif`;
    ctx.fillStyle = pal.ink;
    ctx.fillText((attribution || "").toUpperCase(), w / 2, y + w * 0.055);
    ctx.globalAlpha = a * 0.8;
    ctx.font = `500 ${Math.round(w * 0.028)}px 'Inter', sans-serif`;
    ctx.fillText(endLine, w / 2, y + w * 0.118);
    ctx.restore();
  ...
  function motionSeqFrame(ctx, el) {
    ...
    const stillsDur = imgs.length * per;
    const t = el / 1000; // GLOBAL clock — layers never reset at a cut

    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, w, h);

    if (endCardOn && el >= stillsDur) {
      drawMotionEndCard(ctx, w, h, t, el - stillsDur);
    } else {
    const idx = Math.min(imgs.length - 1, Math.floor(el / per));
    const local = el - idx * per;
    ...
    let a = Math.max(0, Math.min(1, (local - 900) / 900));
    if (idx + 1 < imgs.length) a = Math.min(a, Math.max(0, Math.min(1, (per - local) / 500)));
    if (cap && a > 0) drawMotionCaption(ctx, w, h, cap, a);

    // dialogue overlay: staged call-and-reply on the global clock
    drawMotionDialog(ctx, w, h, t, stillsDur / 1000);

    // dip to black into the end card so the close reads as a cut, not a jump
    if (endCardOn) {
      const f = Math.max(0, Math.min(1, (el - (stillsDur - 450)) / 450));
      if (f > 0) { ctx.save(); ctx.globalAlpha = f; ctx.fillStyle = "#000"; ctx.fillRect(0, 0, w, h); ctx.restore(); }
    }
    }
  ...
  const motionDurMs = () => motionImgs.length * motionPer * 1000 + (endCardOn ? END_CARD_MS : 0);
  ...
  function recordVideoWith(frame = videoFrame, durMs, cues) {
    ...
    try {
      const { stream, snd } = withSound(c.captureStream(30), boomCues, dur / 1000);
      const rec = makeRecorder(stream);
      reportRecording(rec, stream);
      const chunks = [];
      rec.ondataavailable = (e) => e.data.size && chunks.push(e.data);
  ...
  function recordMotionVideo() {
    ...
    // boom at the open, at every cut, on the reply reveal, on the end-card cut,
    // and on the final frame — the audio hits where the eye lands
    const stillsSecs = motionImgs.length * motionPer;
    const cues = [0.2];
    for (let i = 1; i < motionImgs.length; i++) cues.push(i * motionPer);
    if (motionDlg2.trim()) {
      const at = Math.min(motionDlg2At, stillsSecs - 1);
      if (at > 0.5) cues.push(at);
    }
    if (endCardOn) cues.push(stillsSecs + 0.15);
    cues.push(motionDurMs() / 1000 - 0.4);
    recordVideoWith(motionVideoFrame, motionDurMs(), [...new Set(cues)].sort((x, y) => x - y));
  }
        ...
        <div style={S.cols}>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <div style={S.panel}>
              ...
              <button style={{ ...S.ghostBtn, width: "100%", marginTop: 8 }} onClick={testSound}>🔈 Test sound — 5s of the current palette's Sablesound</button>
              {soundStatus && <p style={{ ...S.small, color: "#EDE7DE" }}>{soundStatus}</p>}
              {recInfo && <p style={{ ...S.small, color: "#EDE7DE" }}>{recInfo}</p>}
            ...
            <div style={S.panel}>
              ...
              <label style={S.label}>Captions — one line per still, in order (leave a line blank for no caption)</label>
              <textarea style={{ ...S.input, resize: "vertical" }} rows={3} value={motionCaps} onChange={(e) => setMotionCaps(e.target.value)} />
              <label style={S.label}>Dialogue reveal — the reply is the payoff, so it lands late. Upload CLEAN art (no baked-in text) when using this; Sablepress sets the type.</label>
              <input style={S.input} placeholder="Her line — fades in at 1s, top left" value={motionDlg1} onChange={(e) => setMotionDlg1(e.target.value)} />
              <input style={{ ...S.input, marginTop: 6 }} placeholder="His reply — the hook, top right" value={motionDlg2} onChange={(e) => setMotionDlg2(e.target.value)} />
              <div style={{ display: "flex", gap: 16, marginTop: 8, alignItems: "center", flexWrap: "wrap" }}>
                <div style={{ width: 150 }}>
                  <label style={S.label}>Reply appears at (s)</label>
                  <input type="number" min={2} max={12} step={0.5} style={S.input} value={motionDlg2At} onChange={(e) => setMotionDlg2At(Math.max(2, Math.min(12, +e.target.value || 4.5)))} />
                </div>
                <label style={{ ...S.small, display: "flex", alignItems: "center", gap: 6, margin: 0 }}>
                  <input type="checkbox" checked={endCardOn} onChange={(e) => setEndCardOn(e.target.checked)} /> 🏷 End card — 3s branded close (Hook + title + attribution + CTA)
                </label>
              </div>
              {endCardOn && (
                <div style={{ marginTop: 8 }}>
                  <input style={S.input} placeholder="End card title — e.g. THE GIFT HELD" value={endTitle} onChange={(e) => setEndTitle(e.target.value)} />
                  <input style={{ ...S.input, marginTop: 6 }} placeholder="Closing line — e.g. Free to read on Kindle Unlimited" value={endLine} onChange={(e) => setEndLine(e.target.value)} />
                </div>
              )}
              {fmt.h / fmt.w < 1.7 && (
                <button style={{ ...S.ghostBtn, width: "100%", marginTop: 10, borderColor: "#C27B3F", color: "#E8A96B" }}
                  onClick={() => setFmtName("Video / Story 9:16 (1080×1920)")}>
                  ⚠ Format is {fmtName.split(" (")[0]} — tap to switch to 9:16 before recording. TikTok letterboxes anything narrower.
                </button>
              )}
              ...
              <button style={{ ...S.primaryBtn, width: "100%", marginTop: 12, opacity: recording ? 0.6 : 1 }} onClick={recordMotionVideo} disabled={recording}>
                {recording ? "Working… watch the preview" : `🎬 Record motion sequence (${motionImgs.length || 0} still${motionImgs.length === 1 ? "" : "s"}, ~${motionImgs.length * motionPer + (endCardOn ? 3 : 0)}s)`}
              </button>
