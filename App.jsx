...
function buildSablesound(actx, dest, palKey, durSecs, t0) {
  const preset = SOUND_PRESETS[palKey] || SOUND_PRESETS["Ember (—BORN core)"];
...
STRICT: no watermark, no signature, no modern objects${withText ? ", no text other than the two captions specified above" : ", no text or lettering of any kind"}.`;
  ...
  function testSound() {
    ...
    playThroughElement(dest.stream, 5);
    setSoundStatus(`Playing 5s test (${palName}) — if silent, raise volume with the side buttons.`);
  ...
  function soundClipInto(videoStream, durSecs) {
    ...
    try {
      ...
      playThroughElement(monitorDest.stream, durSecs); // hear it while it records
  ...
  function soundAloud(durSecs, delaySecs) {
    if (!soundOn) return;
    const actx = getSoundCtx();
    if (!actx) return;
    try {
      const dest = actx.createMediaStreamDestination();
      buildSablesound(actx, dest, palName, durSecs, actx.currentTime + (delaySecs || 0));
      playThroughElement(dest.stream, durSecs + (delaySecs || 0));
    } catch (e) { /* silent */ }
  }
        ...
        <div style={S.cols}>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <div style={S.panel}>
              ...
              <label style={{ ...S.small, display: "flex", alignItems: "center", gap: 8, marginTop: 10 }}>
                <input type="checkbox" checked={soundOn} onChange={(e) => setSoundOn(e.target.checked)} />
                <span>🔊 Bake Sablesound into the video — palette-matched dark drone, riser into an impact hit at 2.5s. Applies to all recorded videos. In screen-recording fallback it plays out loud so the recording captures it.</span>
              </label>
