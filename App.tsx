...
// ---- daily comment rounds (88/100): line bank + TikTok search terms per category ----
const COMMENT_BANK = [
  { cat: "Villain / morally-grey", emoji: "🖤", lines: [
    "Soft love asks. Dark love keeps the receipt.",
    "He's not the red flag. He's the whole burning building, and she brought marshmallows.",
    "Some men are warnings. Some are invitations. The genre knows they can be both.",
    "A gentle man would have let her go. Thank god he isn't one.",
    "The villain gets the best lines because he's the only one telling the truth.",
  ], searches: ["morally grey men", "dark romance villain", "book boyfriend dark", "villain edit books"] },
  { cat: "Heartbreak / angst", emoji: "🥀", lines: [
    "The ones who love gently get forgotten gently. This genre understands that.",
    "Grief is just love with nowhere left to put its hands.",
    "She didn't break. She inventoried the damage and sent him the bill.",
    "The cruelest thing he ever did was be kind first.",
  ], searches: ["book angst", "books that broke me", "sad book quotes", "romantasy angst"] },
  { cat: "Genre defense", emoji: "🔥", lines: [
    "We don't read this to feel safe. We read it to be held by something with teeth.",
    "Romance promises a happy ending. Dark romance makes you earn it.",
    "It's not escapism. It's a controlled burn.",
    "People ask why women read this. Wrong question. Ask why we recognize it.",
  ], searches: ["why I read dark romance", "dark romance explained", "dark romance community"] },
  { cat: "Hauls / TBR", emoji: "📚", lines: [
    "A TBR pile isn't clutter. It's a hoard, and we are the dragon.",
    "Buying books and reading books are two different hobbies, and I excel at both. Mostly the first.",
    "The stack grows because hope grows. Let it.",
  ], searches: ["book haul", "tbr pile", "dark romance haul", "bookshelf tour"] },
  { cat: "Reaction / destroyed me", emoji: "😭", lines: [
    "The best books don't end. They just stop letting you in.",
    "If it didn't ruin your sleep, your appetite, and one friendship, was it even a five-star read?",
    "Book hangovers are proof the door was real.",
  ], searches: ["book hangover", "this book destroyed me", "finished this book crying"] },
  { cat: "Spice discourse", emoji: "🌶", lines: [
    "The tension is the spice. The rest is just paperwork.",
  ], searches: ["spicy booktok", "slow burn romance", "spice tolerance books"] },
];

// ---- 🔔 Inner Ring: newsletter CTA + tracked bio links (signups per post) ----
// Ritual: post → Log this post → copy bio link → paste as the TikTok bio link.
// Landing page forwards utm_content into a hidden MailerLite field "post_id";
// count subscribers per post_id in MailerLite and backfill the ledger here.
const RING_URL = "https://lenoresable.com/ring";
const RING_BOOKS = [
  ["GB1", "The Gift Held"], ["GB2", "The Gift Poured"], ["GB3", "The Gift Spent"],
  ["GB4", "The Gift Kept"], ["GB5", "The Gift Woven"], ["SER", "Series / general"],
];
const RING_CTAS = [
  { id: "bonus-scene",
    endCard: "A scene that never made the book.\nInner Ring only. Link in bio.",
    caption: "The deleted scene lives in the Inner Ring — link in bio 🔔",
    pinned: "The scene I cut from this book is in the Inner Ring. Bio link." },
  { id: "first-to-know",
    endCard: "The Ring reads first.\nLink in bio.",
    caption: "Inner Ring gets the next book before anyone. Bio 🔔",
    pinned: "Next —BORN book drops to the Inner Ring first. Link in bio." },
  { id: "behind-canon",
    endCard: "What the gift actually costs —\nthe lore notes are in the Ring.",
    caption: "The canon notes behind this scene are Ring-only. Bio 🔔",
    pinned: "I put the lore behind this moment in the Inner Ring. Bio link." },
  { id: "name-reveal",
    endCard: "You only get a name once.\nThe Ring knows why. Bio.",
    caption: "The rule behind the names — explained in the Inner Ring 🔔",
    pinned: "The naming rule has a cost. Full explanation in the Ring. Bio." },
  { id: "ending-vote",
    endCard: "The Ring votes on what I write next.\nLink in bio.",
    caption: "Inner Ring picks what I write next. Have a say — bio 🔔",
    pinned: "The Ring is voting on the next storyline. Join in the bio." },
  { id: "quiet-door",
    endCard: "There is a quieter room\nfor readers like you. Bio.",
    caption: "For the ones who reread the last chapter — Inner Ring, bio 🔔",
    pinned: "If this book sat with you, the Inner Ring is your room. Bio." },
];

// ---- 💬 comment-bait caption lines: comments are the cheapest ranking signal ----
const BAIT_LINES = [
  "Whose line hurts more — hers or his? One word, comments.",
  "Be honest: would you have stayed after that line?",
  "Which word would break you first? Drop it below.",
  "Read the last line again. Now tell me you're fine.",
  "Team her or team him after this? Choose in the comments.",
  "What would YOU have answered? Wrong answers only.",
  "Rate the damage 1–10. Comments open.",
  "Finish his sentence. I'll pin the best one.",
];

// ---- 📚 binge ladder: quotes sell mood, read-through pays the bills ----
const LADDER = [
  { id: "START", hook: "Seen the lines everywhere?", line: "They all start where the gift is first held. One book. One debt of the heart.", cta: "Start with The Gift Held — Book One, on Kindle Unlimited." },
  { id: "GB1→GB2", hook: "You finished The Gift Held?", line: "What was held must be poured. Book Two is where it spills.", cta: "The Gift Poured — out now on Kindle Unlimited." },
  { id: "GB2→GB3", hook: "The pouring was the easy part.", line: "Everything poured gets spent. Book Three counts the cost.", cta: "The Gift Spent — on Kindle Unlimited." },
  { id: "GB3→GB4", hook: "You thought spending was the end.", line: "What survives the spending must be kept. Book Four holds the door.", cta: "The Gift Kept — on Kindle Unlimited." },
  { id: "GB4→GB5", hook: "One book left.", line: "Held, poured, spent, kept — now every thread gets woven.", cta: "The Gift Woven — the ending, on Kindle Unlimited." },
  { id: "GB→DEBT", hook: "Finished GIFTBORN?", line: "If GIFTBORN broke you, DEBTBORN collects.", cta: "DEBTBORN, Book One — same universe, a new price to pay." },
];

// ---- 💬 reply-card attributions ----
const REPLY_OPENERS = [
  "— Lenore, answering honestly",
  "— Lenore, who wrote it and still isn't over it",
  "— Lenore, no spoilers, only wounds",
  "— Lenore Sable",
];

// ---- 🖋 Lenore Voice: universe-wide brand-voice writer + Ring exclusives ----
const LV_PROMPT = `You are the brand voice of Lenore Sable, indie author of dark romantasy — the —BORN Universe, marketed on TikTok (@lenore.sable) and Kindle Unlimited, with the Inner Ring newsletter as the home fire.

THE —BORN UNIVERSE (cosmological spine — each Age turns on one law):
GIFTBORN — Take. Series: The Gift Held / Poured / Spent / Kept / Woven. Every gift has a price; every price is paid in person.
DEBTBORN — Give. Five books. What is owed binds tighter than what is loved.
COLDBORN — Keep. Five books (incl. The Held Inch, The Open Hand). What you hold back from the cold is what the cold comes for.
BLOODBORN — Open. Next Age, in development. What is opened cannot be unshed.
ASHBORN — Belong. Five books. What survives the fire belongs to it.
TIDEBORN — Four books, complete. What the tide takes, it takes twice.
TOLLBORN — In development (The Uncharted Toll). Every crossing is counted.

THE VOICE:
- Dark, intimate, costly. Second-person ache is the signature register.
- Recursive, litany-like cadence used sparingly — one turn of the knife, not five.
- Firelight and elemental imagery keyed to the series' law (gift/debt/cold/blood/ash/tide/toll).
- Never cute, never quirky, never exclamation-heavy. No more than one emoji, ever, usually none.
- Devotion with teeth. Morally weighted love. Yearning that costs.

FICTION RULES:
- Bonus scenes: 400–600 words, full prose, complete arc-in-miniature, ends on a held breath. Written as an Inner Ring exclusive.
- Microfiction teasers: 90–180 words, single image or exchange, spoiler-safe, built to be read aloud over firelight footage.
- Honor any canon notes provided EXACTLY. Where canon is not given, stay atmospheric and non-committal — never invent named plot events, deaths, or reveals.
- Spoiler-safe means: mood, cost, and voice — not outcomes.

MARKETING RULES:
- TikTok captions: 1–2 line hook, then 3–6 niche hashtags (#romantasy #darkromantasy #booktok #kindleunlimited plus one specific). Soft CTA tail to the series or the Inner Ring (link in bio).
- Inner Ring emails: subject under 45 characters, body under 120 words, one mention of "the Inner Ring", signed "— Lenore".
- Blurbs: KU-commercial. Open on the wound, not the worldbuilding. End on an irresistible cost.
- Comment replies: warm but in-world, 1–2 sentences, never salesy.

OUTPUT FORMAT — CRITICAL:
Respond ONLY with valid JSON, no markdown fences, no preamble:
{"variants": [{"label": "short 2-4 word angle name", "text": "the copy"}, ...]}
Fiction modes (bonus scene, microfiction): return exactly 1 variant. All other modes: exactly 3 variants.`;

const LV_SERIES = ["GIFTBORN", "DEBTBORN", "COLDBORN", "BLOODBORN", "ASHBORN", "TIDEBORN", "TOLLBORN", "New / other"];
const LV_MODES = [
  { id: "scene", name: "Bonus scene", fiction: true, hint: "The moment: who, where, what it costs" },
  { id: "micro", name: "Microfiction", fiction: true, hint: "One image or theme to spin 90–180 words from" },
  { id: "caption", name: "TikTok caption", hint: "What's the video?" },
  { id: "hooks", name: "Hook lines", hint: "Scene, theme, or book to hook from" },
  { id: "email", name: "Ring email", hint: "The occasion (release, teaser, behind-the-veil)" },
  { id: "blurb", name: "Blurb", hint: "Which book, and the core wound of it" },
  { id: "reply", name: "Comment reply", hint: "Paste the comment you're replying to" },
];
const LV_TASKS = {
  scene: "Write 1 Inner Ring exclusive bonus scene per the fiction rules (400–600 words). Label it with the scene's angle.",
  micro: "Write 1 spoiler-safe microfiction teaser per the fiction rules (90–180 words).",
  caption: "Write 3 TikTok captions per the marketing rules.",
  hooks: "Write 3 opening hook lines (on-screen text or first spoken line). Each under 15 words. No hashtags.",
  email: "Write 3 Inner Ring email drafts. Each variant text must contain 'Subject: ...' on its first line, then the body.",
  blurb: "Write 3 KU blurb drafts per the marketing rules. 80–130 words each.",
  reply: "Write 3 reply options to this comment per the comment-reply rules.",
};
...
function drawCard(ctx, w, h, pal, seed, content, t = 0, fade = 1) {
  paintBackground(ctx, w, h, pal, seed, t);
  drawCardText(ctx, w, h, pal, content, fade);
}

// text layer shared by the painted card and the Firecard — one typography, two backgrounds
function drawCardText(ctx, w, h, pal, { hook, quote, attribution, handle }, fade = 1) {
  const margin = w * 0.1;
...
// ---------- Firecard: procedural signature opener ----------
// One brand look, never the same pixels twice. Layout (flame count, candle
// positions, ember field) comes from the seed; the flicker runs off the clock
// on seed-derived phases. Same seed = reproducible frame; new seed = new
// pixels — which is what keeps the dedupe fingerprint fresh post after post.
const FIRE_COLS = {
  "Hearth fire": { flame: ["#8F2B12", "#E8752B", "#FFE3B0"], glow: null, sparks: 46 },
  "Candlelight": { flame: ["#C4571E", "#F5A93F", "#FFF3D6"], glow: null, sparks: 14 },
  "Ember drift": { flame: null, glow: null, sparks: 110 },
  "Cold flame": { flame: ["#173E78", "#3F8FD0", "#E8F7FF"], glow: "#7FC4E8", sparks: 28 },
};

function drawFlame(ctx, x, baseY, hgt, wdt, t, ph, cols) {
  // three nested teardrops; the tip sways on two mixed sine bands so the
  // flicker never visibly loops
  ctx.save();
  ctx.globalCompositeOperation = "lighter";
  for (let l = 0; l < 3; l++) {
    const k = 1 - l * 0.34;
    const hh = hgt * k * (0.92 + 0.08 * Math.sin(t * 5.1 + ph + l));
    const ww = wdt * k;
    const sway = Math.sin(t * 6.7 + ph) * wdt * 0.22 + Math.sin(t * 11.3 + ph * 2.1) * wdt * 0.1;
    const tipX = x + sway * (1 - l * 0.25), tipY = baseY - hh;
    ctx.beginPath();
    ctx.moveTo(x - ww / 2, baseY);
    ctx.bezierCurveTo(x - ww / 2, baseY - hh * 0.42, tipX - ww * 0.16, tipY + hh * 0.24, tipX, tipY);
    ctx.bezierCurveTo(tipX + ww * 0.16, tipY + hh * 0.24, x + ww / 2, baseY - hh * 0.42, x + ww / 2, baseY);
    ctx.closePath();
    ctx.fillStyle = cols[l] + (l === 0 ? "66" : l === 1 ? "8C" : "E0");
    ctx.shadowColor = cols[1];
    ctx.shadowBlur = l === 0 ? 34 : 16;
    ctx.fill();
  }
  ctx.restore();
}

function drawFireScene(ctx, w, h, pal, fseed, t, variant) {
  const rnd = mulberry32(fseed);
  const V = FIRE_COLS[variant] || FIRE_COLS["Hearth fire"];
  const glowCol = V.glow || pal.accent;
  ctx.fillStyle = pal.bg;
  ctx.fillRect(0, 0, w, h);

  // deep colour washes (two — lighter than the painter's four)
  for (let i = 0; i < 2; i++) {
    const x = rnd() * w, y = rnd() * h * 0.7, r = (0.4 + rnd() * 0.4) * w;
    const g = ctx.createRadialGradient(x, y, 0, x, y, r);
    g.addColorStop(0, pal.tones[1 + Math.floor(rnd() * 2)] + "3A");
    g.addColorStop(1, "transparent");
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, w, h);
  }

  // ground glow: the light source, breathing
  const breathe = 0.8 + 0.2 * Math.sin(t * 1.7) + 0.06 * Math.sin(t * 6.3);
  const gg = ctx.createRadialGradient(w / 2, h * 0.86, 0, w / 2, h * 0.86, w * 0.75 * breathe);
  gg.addColorStop(0, glowCol + "55");
  gg.addColorStop(0.4, glowCol + "22");
  gg.addColorStop(1, "transparent");
  ctx.fillStyle = gg;
  ctx.fillRect(0, 0, w, h);

  if (variant === "Hearth fire" || variant === "Cold flame") {
    const baseY = h * 0.9;
    // log silhouettes
    ctx.save();
    ...
    [-1, 1].forEach((s2) => {
      ctx.save();
      ctx.translate(w / 2 + s2 * w * 0.07, baseY + h * 0.012);
      ctx.rotate(s2 * (0.16 + rnd() * 0.08));
      const lw = w * (0.2 + rnd() * 0.05), lh2 = h * 0.02;
      ctx.beginPath();
      ctx.roundRect(-lw / 2, -lh2 / 2, lw, lh2, lh2 / 2);
      ctx.fill();
      ctx.restore();
    });
    ctx.restore();
    // flame cluster: 3–5 tongues, tallest in the middle
    const n = 3 + Math.floor(rnd() * 3);
    for (let i = 0; i < n; i++) {
      const fx = w / 2 + (rnd() - 0.5) * w * 0.16;
      const hgt = h * (0.13 + rnd() * 0.1) * (i === Math.floor(n / 2) ? 1.35 : 1);
      const wdt = w * (0.05 + rnd() * 0.04);
      drawFlame(ctx, fx, baseY, hgt, wdt, t, rnd() * 20, V.flame);
    }
  }

  if (variant === "Candlelight") {
    const n = 3 + Math.floor(rnd() * 3); // 3–5 candles per seed
    // the table: a darker band so the candles sit on something
    ctx.fillStyle = "#00000055";
    ctx.fillRect(0, h * 0.82, w, h * 0.18);
    for (let i = 0; i < n; i++) {
      const cx = w * (0.2 + ((i + 0.5) / n) * 0.6) + (rnd() - 0.5) * w * 0.04;
      const ch = h * (0.07 + rnd() * 0.09);
      const cw = w * (0.022 + rnd() * 0.012);
      const cy = h * 0.82 + (rnd() - 0.5) * h * 0.008;
      const ph = rnd() * 20;
      // halo, each candle breathing on its own phase
      const ha = 0.3 + 0.12 * Math.sin(t * (1.3 + rnd()) + ph);
      const hg = ctx.createRadialGradient(cx, cy - ch, 0, cx, cy - ch, w * 0.2);
      hg.addColorStop(0, glowCol + Math.round(ha * 255).toString(16).padStart(2, "0"));
      hg.addColorStop(1, "transparent");
      ctx.fillStyle = hg;
      ctx.fillRect(cx - w * 0.22, cy - ch - w * 0.22, w * 0.44, w * 0.44);
      // wax body with a lit edge
      const bg = ctx.createLinearGradient(cx - cw / 2, 0, cx + cw / 2, 0);
      bg.addColorStop(0, "#B9A57E");
      bg.addColorStop(0.5, "#EFE2C4");
      bg.addColorStop(1, "#8F7C5C");
      ctx.fillStyle = bg;
      ctx.beginPath();
      ctx.roundRect(cx - cw / 2, cy - ch, cw, ch, cw * 0.3);
      ctx.fill();
      // seeded wax drip
      if (rnd() > 0.35) {
        const dx = cx - cw / 2 + rnd() * cw, dl = ch * (0.2 + rnd() * 0.3);
        ctx.fillStyle = "#EFE2C4CC";
        ctx.beginPath();
        ctx.roundRect(dx - cw * 0.09, cy - ch, cw * 0.18, dl, cw * 0.09);
        ctx.fill();
      }
      // wick + flame
      ...
      ctx.beginPath();
      ctx.moveTo(cx, cy - ch);
      ctx.lineTo(cx, cy - ch - h * 0.006);
      ctx.stroke();
      drawFlame(ctx, cx, cy - ch - h * 0.004, h * (0.03 + rnd() * 0.012), cw * 1.5, t, ph, V.flame);
    }
  }

  // ember field — concentrated over the fire, spreading as it rises
  ctx.save();
  ctx.globalCompositeOperation = "lighter";
  const focusX = variant === "Candlelight" ? null : w / 2;
  for (let i = 0; i < V.sparks; i++) {
    const py = (((rnd() * h) - t * (34 + rnd() * 70)) % h + h) % h;
    const lift = 1 - py / h;
    const px = focusX == null
      ? rnd() * w
      : focusX + (rnd() - 0.5) * w * (0.14 + lift * 0.7) + Math.sin(t * 0.9 + i) * w * 0.01;
    const big = rnd() > 0.88;
    const r = big ? 2.4 + rnd() * 3 : 0.8 + rnd() * 1.6;
    ctx.globalAlpha = (big ? 0.16 : 0.38 + rnd() * 0.35) * (0.55 + 0.45 * Math.sin(t * 2.2 + i));
    ctx.shadowColor = glowCol;
    ctx.shadowBlur = big ? 16 : 8;
    ctx.fillStyle = big ? glowCol : (V.flame ? V.flame[2] : pal.tones[4]);
    ctx.beginPath();
    ctx.arc(px, py, r, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  // focal glow behind the text block
  const fg = ctx.createRadialGradient(w / 2, h * 0.44, 0, w / 2, h * 0.44, w * 0.6);
  fg.addColorStop(0, glowCol + "20");
  fg.addColorStop(1, "transparent");
  ctx.fillStyle = fg;
  ctx.fillRect(0, 0, w, h);
  ...
  // smoke wisps, ghosted
  for (let i = 0; i < 3; i++) {
    let x = w * (0.3 + rnd() * 0.4), y = h * (0.75 + rnd() * 0.1);
    ctx.strokeStyle = pal.tones[3] + "0A";
    ctx.lineWidth = 70 + rnd() * 90;
    ctx.lineCap = "round";
    ctx.beginPath();
    ctx.moveTo(x, y);
    for (let s = 0; s < 5; s++) {
      x += (rnd() - 0.5) * w * 0.25;
      y -= h * 0.09 + rnd() * h * 0.05;
      ctx.lineTo(x + Math.sin(t * 0.6 + i * 2) * 16, y);
    }
    ctx.stroke();
  }

  // film grain with a faint flicker
  const grnd = mulberry32(Math.floor(t * 8) + fseed);
  const grains = Math.floor((w * h) / 3600);
  for (let i = 0; i < grains; i++) {
    ctx.fillStyle = grnd() > 0.5 ? "#FFFFFF06" : "#0000000C";
    ctx.fillRect(grnd() * w, grnd() * h, 1.5, 1.5);
  }

  // breathing vignette
  const inner = Math.min(w, h) * (0.3 + 0.02 * Math.sin(t * 0.6));
  const v = ctx.createRadialGradient(w / 2, h / 2, inner, w / 2, h / 2, Math.max(w, h) * 0.72);
  v.addColorStop(0, "transparent");
  v.addColorStop(1, "#000000C4");
  ctx.fillStyle = v;
  ctx.fillRect(0, 0, w, h);
}

function drawFireCard(ctx, w, h, pal, fseed, content, t = 0, fade = 1, variant = "Hearth fire") {
  drawFireScene(ctx, w, h, pal, fseed, t, variant);
  drawCardText(ctx, w, h, pal, content, fade);
}
...
export default function App() {
  ...
  // ---- Firecard (signature opener) state ----
  const [fireVariant, setFireVariant] = useState("Hearth fire");
  const [fireSeed, setFireSeed] = useState(() => Math.floor(Math.random() * 100000));
  const [fireReseed, setFireReseed] = useState(true); // fresh pixels every export
  const [fireSecs, setFireSecs] = useState(6);
  const [fireLead, setFireLead] = useState(false); // open Motion sequences with the Firecard
  ...
  const STAGINGS = [
    // — the proven format: two faces inches apart, a light source between them —
    "facing each other inches apart across a rough wooden table, a single candle between them, its flame lighting both faces, unresolved tension",
    "seated inches apart beside a dying campfire in a dark forest at night, ember glow on both faces, sparks rising between them",
    "close together in a torchlit stone corridor, her back to the wall, his hand braced on the stone beside her head, faces nearly touching",
    "soaked under night rain at a castle gate, faces inches apart, cold moonlight through the downpour",
    "in a candlelit library, faces close over a single open book, warm gold lamplight, dust motes in the light",
    "dancing close in an empty candlelit great hall, her hand in his, faces nearly touching, the candles guttering",
    "sheltering under one cloak in a storm, pressed close, faces inches apart in swinging lantern light",
    "she holds a lantern up between their faces in a midnight forest, the light trembling, neither stepping back",
    // — wider stagings for variety —
    ...
    // — expansion set: 16 more —
    "facing each other across a narrow stone bridge at night, a lantern hung midway between them, black water below",
    "close in a ruined chapel, moonlight through a broken rose window falling between their faces",
    "she binds a wound on his forearm by firelight, both heads bent close over the work, neither speaking",
    "in a stable at night among sleeping horses, a single hung lamp, straw-gold light on both faces",
    "facing each other in a wheat field at dusk, the wind moving the wheat between them",
    "close at a rain-streaked window, one candle behind them, their reflections almost touching in the glass",
    "he lifts her down from a horse in a torchlit courtyard, hands at her waist, the moment held too long",
    "kneeling on opposite sides of a small grave at first light, a single wildflower laid between them",
    "in a boat on black water at night, she holds the lantern, he rows, faces lit from below",
    "backed into a market alcove at night hiding from passing torches, pressed close, breath held",
    "she reads aloud by candlelight while he sits on the floor at her feet, his head near her knee",
    "facing each other on a cliff path in high wind, her cloak whipping toward him like a reaching hand",
    "in a forge at night, ember light and rising sparks, she holds the iron steady while he strikes",
    "close beneath a stone archway as snow falls beyond it, a shared loaf broken between them",
    "she stitches his torn shirt by lamplight while he watches her hands, the silence heavier than speech",
    "standing back to back in torchlight, blades lowered, both breathing hard, the danger just past",
  ];
  ...
  // ---- fixed quote bank: 16 original in-voice lines per series (NOT manuscript excerpts) ----
  // Used by the flipcard set builder after starred/mined lines, before the seeded composer.
  const QUOTEBANK = {
    "Ember (—BORN core)": [
      "The ledger never forgets a name. It only waits for the hand that wrote it.",
      "She paid what was owed. No one told her the interest was herself.",
      "Some debts are not collected. They are inherited, and they know your face.",
      "He kept the bargain the way you keep a wound: covered, and always aching.",
      "What was owed in the dark must be paid in the dark.",
      "You do not carry a burden that long without learning to call it yours.",
      "The bargain was simple. Everything after it was not.",
      "She counted what she'd given. The number frightened her less than who was counting with her.",
      "Every borrowed name comes due. Hers came due at his door.",
      "He said the debt was settled. The ash said otherwise.",
      "There is a door past the last door. That is where the owing lives.",
      "What you keep when no one is watching is the only thing you truly own.",
      "The burden did not break her. It introduced itself, and stayed.",
      "Some ledgers are written in ink. The old ones are written in people.",
      "She was raised to pay her way. No one said the road would charge in years.",
      "The bargain took nothing she'd miss. That was the first lie.",
    ],
    "Gilt (GIFTBORN)": [
      "A gift held too long stops being a gift. It becomes a hostage.",
      "She poured herself out for them. They brought cups. They never brought her one.",
      "What is freely given can still be counted. He counted everything.",
      "The gift was never the gold. The gift was that she stayed.",
      "You can spend a fortune. You can spend a life. Only one is refunded.",
      "He gave before it was asked. That is how you buy a person without a price.",
      "What was kept against the taking grew heavier than the taking would have been.",
      "Open hands, she learned, are the hardest thing to keep open.",
      "Everything woven can be unwoven. She held the thread and did not pull.",
      "They called it generosity. It had teeth, and it knew her name.",
      "A gift returned twice over is not gratitude. It is a message.",
      "She kept what he gave her in the one place he couldn't audit: the marrow.",
      "The poured cup empties. The pouring hand forgets. The cup remembers.",
      "He spent her carefully, the way misers spend: pretending it costs them.",
      "What you give to be loved is a loan. What you give anyway is a gift.",
      "The weaving took years. The unraveling took one honest sentence.",
    ],
    "Frost (COLDBORN)": [
      "The cold does not hate you. It simply does not stop.",
      "She held the inch. Winters change hands over less.",
      "What the keeper keeps, the keeper becomes.",
      "Three months south, they still speak of warmth as if it were owed.",
      "The last bell rings for whoever is left to hear it. She was left.",
      "He learned to endure the way ice learns the shape of the shore.",
      "Past the frost line there are no promises. Only what you carried in.",
      "The long winter asks one question, daily: what will you keep?",
      "Fires fail. Keeping does not. That is why keeping is colder.",
      "She answered the cold the only way it respects: by remaining.",
      "The thaw reveals everything the snow was kind enough to hide.",
      "What outlasts the winter is not the strongest thing. It is the most stubborn.",
      "In the keeper's silence there was room for exactly one other person.",
      "The held inch was never ground. It was her.",
      "Cold teaches arithmetic: what you save, what you spend, what you become.",
      "He kept the door. She kept the fire. The winter kept the score.",
    ],
    "Blood (BLOODBORN)": [
      "The blood remembers what the family agreed to forget.",
      "Belonging has a price. The old houses pay it in daughters.",
      "The vow was made before her birth. It still knew where to find her.",
      "You can leave the family table. The table does not leave you.",
      "Down the long line, every wound arrives wearing a familiar face.",
      "The lineage demanded a keeper. It did not ask if she was willing.",
      "What binds you at the naming binds you at the grave.",
      "The past knocked politely, once. After that it used its key.",
      "He carried the old wound the way heirs carry everything: as if chosen.",
      "Under the same roof, love and debt learn to share a bed.",
      "The blood claimed her quietly, the way rivers claim their banks.",
      "Some inheritances are counted in land. Hers was counted in silences.",
      "The family remembered everything except how the remembering started.",
      "A vow outlives the mouth that made it. That is its whole design.",
      "She was born belonging. It took years to learn what it cost.",
      "The price of belonging is paid in installments. The last one is yourself.",
    ],
    "Ash (ASHBORN)": [
      "The fire took the house. The ash kept the shape of it.",
      "What the burning left was not nothing. It was the true inventory.",
      "She rebuilt on the whole ground because the whole ground was honest now.",
      "In the grey morning, everything unsaid was finally visible.",
      "The smoke cleared. The reasons didn't.",
      "Where the house stood, she stood. That was the beginning.",
      "He carried the aftermath the way you carry sleep: everywhere, unseen.",
      "New rafters over old ground. That is what forgiveness looks like from below.",
      "The ash does not accuse. It just refuses to be anything else.",
      "What survives a burning has already answered the only question that matters.",
      "She reclaimed the ruin room by room, and herself the same way.",
      "The fire was fast. The rebuilding was the rest of her life.",
      "After the burning, every warm thing had to earn its trust again.",
      "They mourned the house. She mourned the version of them it held.",
      "The grey morning asked nothing. That was its mercy.",
      "You do not rebuild to forget. You rebuild so remembering has walls.",
    ],
    "Tide (TIDEBORN)": [
      "The sea keeps what it takes. The harbor keeps what refuses to be taken.",
      "At low water the truth of the coast shows. People are the same.",
      "The tide returned everything except what she'd actually asked for.",
      "The refusal was small. So is a seawall, from far enough away.",
      "What the sea kept, the town agreed not to name.",
      "Beyond the seawall the rules are older and they do not negotiate.",
      "When the bell rings, everyone remembers what they owe the water.",
      "The last mooring held. That is the whole story of some lives.",
      "She answered the storm before it named itself. That is the only time you can.",
      "The turning sky promised nothing. She sailed anyway. That was the promise.",
      "Harbors are made by refusing the sea, politely, forever.",
      "The tide remembered her the way water remembers: by returning.",
      "What refuses to sink learns strange ways of floating.",
      "The sea asked twice. The second asking is not a question.",
      "He held the mooring line the way you hold a promise: past sense.",
      "Low water hides nothing. That is why the town preferred the flood.",
    ],
    "Toll (TOLLBORN)": [
      "Every crossing has a toll. The honest ferrymen tell you before the far bank.",
      "The coast eats maps. That is why the old pilots draw in memory.",
      "She paid the toll in the only coin the water takes: her true name.",
      "The chart ended. She did not.",
      "At the water's edge, every traveler is exactly what they carry.",
      "The crossing changed nothing. The paying did.",
      "Some tolls are paid at the gate. The real ones are paid mid-water.",
      "A true name given at the wrong crossing belongs to the crossing.",
      "He charted the coast in his own hand so the lies would at least be his.",
      "The toll-keeper never chases. The road ahead does the collecting.",
      "Where the coast eats maps, the uncharted is not empty. It is patient.",
      "Every crossing remembers its price. Cross twice and it remembers you.",
      "She named the water. The water, being older, named her back.",
      "The map was wrong in the way maps are wrong: on purpose, once.",
      "You can refuse the toll. The far bank simply stops existing.",
      "The last crossing costs what the first sixteen were saving you for.",
    ],
  };
  ...
  function robustCopy(text, okMsg = "Prompt copied — paste it into ChatGPT.") {
    // Try the modern API first; fall back to execCommand, which works in webviews that block navigator.clipboard
    const failMsg = "Copy blocked — tap the text below to select it, then copy manually.";
    const legacy = () => {
  ...
  // ---- 🖋 Lenore Voice state ----
  const [lvMode, setLvMode] = useState("scene");
  const [lvSeries, setLvSeries] = useState("GIFTBORN");
  const [lvInput, setLvInput] = useState("");
  const [lvCanon, setLvCanon] = useState("");
  const [lvOut, setLvOut] = useState(null);
  const [lvBusy, setLvBusy] = useState(false);
  const [lvErr, setLvErr] = useState("");

  async function generateLenore() {
    if (!lvInput.trim() || lvBusy) return;
    setLvBusy(true); setLvErr(""); setLvOut(null);
    try {
      const task = LV_TASKS[lvMode];
      const userMsg = `${task}\n\nSeries: ${lvSeries}` +
        (lvCanon.trim() ? `\n\nCanon notes (honor exactly):\n${lvCanon.trim()}` : "") +
        `\n\nContext from Lenore: ${lvInput.trim()}`;
      ...
      if (!parsed.variants || !parsed.variants.length) throw new Error("empty");
      setLvOut(parsed.variants);
    } catch (e) {
      setLvErr("AI unavailable — this module needs the in-Claude environment (or an API relay) to write. Reword and retry, or run inside Claude.");
    }
    setLvBusy(false);
  }
  ...
  const [flipNo, setFlipNo] = useState(1);
  ...
  function buildCampaign() {
    ...
    // quote: starred/mined first, biased toward proven performers
    const quotePool = [...favLines, ...library.flatMap((b) => b.quotes || [])];
    const { sIdx: perfS, qMap: perfQ } = perfScores();
    const q = quotePool.length ? weightedSample(quotePool, (x) => perfQ[x] || 0, 1, rnd)[0] : offlineQuotes(5, n * 104729 + 7)[0];

    // three distinct stagings, seeded, biased toward proven performers
    const idx = weightedSample(STAGINGS.map((_, i) => i), (i) => perfS[i] || 0, STAGINGS.length, rnd);
...
${BAIT_LINES[(n - 1) % BAIT_LINES.length]}
...
${KIT_TAGS[platform](seriesTag)} #slowburn #newbooks`);
    ...
    setLastBuild({ fmt: "campaign", quote: q, stagings: idx.slice(0, 3) });
  ...
  // ---- flipbook engine: frame-by-frame animation → hard cuts, ping-pong loop ----
  function runFlipbookVideo(imgs, fps = 6, loops = 3) {
    const c = canvasRef.current, ctx = c.getContext("2d");
    // Ping-pong order (1..8..2) hides the loop seam; contain-fit, zero zoom/drift
    // so AI frames stay pixel-aligned and captions never move.
    const seq = [...imgs, ...imgs.slice(1, -1).reverse()];
    const frameMs = 1000 / fps;
    const cycle = seq.length * frameMs;
    const holdMs = 3000; // end on the FIRST frame, held 3s — the closing shot / freeze frame
    const dur = cycle * loops + holdMs;
    const drawFrame = (el) => {
      const inLoop = Math.min(el, cycle * loops - 1);
      const idx = el >= cycle * loops ? 0 : Math.floor((inLoop % cycle) / frameMs);
      const im = seq[idx];
      ctx.fillStyle = "#000";
      ctx.fillRect(0, 0, fmt.w, fmt.h);
      const s = Math.min(fmt.w / im.width, fmt.h / im.height);
      const dw = im.width * s, dh = im.height * s;
    ...
    const canMp4 = typeof MediaRecorder !== "undefined" && typeof c.captureStream === "function"
      && MediaRecorder.isTypeSupported && MediaRecorder.isTypeSupported("video/mp4");
    if (!canMp4) {
      alert(`Flipbook: ${imgs.length} frames at ${fps}fps, ~${Math.round(dur / 1000)}s.\n\n1. Start iOS screen recording\n2. Tap OK — 3-second countdown, then it plays\n3. Stop, trim in Photos.`);
      setRecording(true);
      const pre = 3000;
      const t0 = performance.now();
      const loop = (now) => {
    ...
    try {
      const { stream, snd } = withSound(c.captureStream(30), [0.2, dur / 1000 - 0.4]);
      let rec;
      try { rec = new MediaRecorder(stream, { mimeType: "video/mp4", videoBitsPerSecond: 8_000_000 }); }
      catch { rec = new MediaRecorder(stream); }
      const chunks = [];
  ...
  // ---- one-tap post-live kit: description + pinned comment ----
  const PIN_QUESTIONS = [
    "Whose line hit harder — hers or his? 🌑",
    "Would you have asked, or kept walking? Be honest.",
    "Which of them is lying? Wrong answers only. 🌑",
    "Tell me you wouldn't answer like that.",
    "Would you want the truth, or the kinder version? 🌑",
    "Whose side are you on here? Choose.",
  ];
  const [postLiveDesc, setPostLiveDesc] = useState("");
  const [postLivePin, setPostLivePin] = useState("");
...
${BAIT_LINES[dayOfYear % BAIT_LINES.length]}
...
${KIT_TAGS[platform](seriesTag)} #slowburn #newbooks`;
    setPostLiveDesc(txt);
    robustCopy(txt, "Description + hashtags copied — paste into the post.");
  }

  function tapPinnedComment() {
    const n = Math.max(1, Math.floor(campaignNo) || 1);
    const series = (attribution.split(",")[0] || "").trim() || "the —BORN Universe";
    const txt = PIN_QUESTIONS[(n - 1) % PIN_QUESTIONS.length];
    setPostLivePin(`${txt}

— When someone asks where it's from, reply:
${ctaSub} — ${series}. Link in bio.`);
    robustCopy(txt, "Pinned comment copied — post it, then long-press → Pin.");
  }

  // ---- daily comment rounds: rotate lines by day-of-year, advance on each tap ----
  const dayOfYear = Math.floor((Date.now() - new Date(new Date().getFullYear(), 0, 0)) / 86400000);
  const [cmtTaps, setCmtTaps] = useState({});
  const [cmtLast, setCmtLast] = useState("");

  function tapComment(ci) {
    const bank = COMMENT_BANK[ci];
    const taps = cmtTaps[ci] || 0;
    const line = bank.lines[(dayOfYear + taps) % bank.lines.length];
    setCmtTaps({ ...cmtTaps, [ci]: taps + 1 });
    setCmtLast(`${bank.emoji} ${bank.cat}:\n${line}`);
    robustCopy(line, "Comment copied — paste on a fresh (<6h) video from a bigger account. Never a link.");
  }

  function openSearch(term) {
    window.open(`https://www.tiktok.com/search/video?q=${encodeURIComponent(term)}`, "_blank");
  }

  // ---- 🔔 Inner Ring: CTA rotation + tracked link + signup ledger ----
  const [ringBook, setRingBook] = useState("GB2");
  const [ringSlot, setRingSlot] = useState(new Date().getHours() < 14 ? "AM" : "PM");
  const [ringTaps, setRingTaps] = useState(0);
  const [ringLedger, setRingLedger] = useState([]);
  const [ringLast, setRingLast] = useState("");
  const [ringLoaded, setRingLoaded] = useState(false);
  useEffect(() => {
    storage.get("sablepress:ring").then((v) => {
      if (v) { try { setRingLedger(JSON.parse(v.value)); } catch { /* fresh */ } }
      setRingLoaded(true);
    });
  }, []);
  useEffect(() => {
    if (ringLoaded) storage.set("sablepress:ring", JSON.stringify(ringLedger));
  }, [ringLedger, ringLoaded]);

  const ringCta = RING_CTAS[(dayOfYear + ringTaps) % RING_CTAS.length];
  const ringPostId = (() => {
    const d = new Date();
    return `${String(d.getFullYear()).slice(2)}${String(d.getMonth() + 1).padStart(2, "0")}${String(d.getDate()).padStart(2, "0")}-${ringSlot}-${ringBook}`;
  })();
  const ringLink = `${RING_URL}?${new URLSearchParams({
    utm_source: "tiktok", utm_medium: "bio", utm_campaign: "giftborn", utm_content: ringPostId,
  }).toString()}`;

  function ringCopy(kind) {
    const map = {
      link: [ringLink, "Bio link copied — paste it as the TikTok bio link when you post."],
      caption: [ringCta.caption, "Caption tail copied — add it under the description."],
      pinned: [ringCta.pinned, "Ring pinned comment copied — post it, then long-press → Pin."],
      endcard: [ringCta.endCard, "End-card text copied — bake it onto the final frame."],
    };
    const [txt, msg] = map[kind];
    setRingLast(txt);
    robustCopy(txt, msg);
  }
  function ringLog() {
    if (ringLedger.some((r) => r.postId === ringPostId)) {
      setRingLast(`Already logged: ${ringPostId}`);
      return;
    }
    setRingLedger([{ postId: ringPostId, date: new Date().toISOString().slice(0, 10),
      slot: ringSlot, book: ringBook, ctaId: ringCta.id, signups: 0 }, ...ringLedger]);
    setRingLast(`Logged ${ringPostId} — now copy the bio link and paste it on TikTok.`);
  }
  function ringSetSignups(id, v) {
    setRingLedger(ringLedger.map((r) => r.postId === id
      ? { ...r, signups: Math.max(0, parseInt(v || 0, 10) || 0) } : r));
  }
  function ringRemove(id) { setRingLedger(ringLedger.filter((r) => r.postId !== id)); }
  const ringTotal = ringLedger.reduce((s, r) => s + r.signups, 0);
  const ringAM = ringLedger.filter((r) => r.slot === "AM").reduce((s, r) => s + r.signups, 0);
  const ringPM = ringLedger.filter((r) => r.slot === "PM").reduce((s, r) => s + r.signups, 0);
  const ringBest = (() => {
    const by = {};
    ringLedger.forEach((r) => { by[r.ctaId] = (by[r.ctaId] || 0) + r.signups; });
    const top = Object.entries(by).sort((a, b) => b[1] - a[1])[0];
    return top && top[1] > 0 ? `${top[0]} (${top[1]})` : "—";
  })();
  function ringCsv() {
    const csv = "post_id,date,slot,book,cta,signups\n" + ringLedger.map((r) =>
      [r.postId, r.date, r.slot, r.book, r.ctaId, r.signups].join(",")).join("\n");
    robustCopy(csv, "Ledger copied as CSV.");
  }

  // ---- 📈 performance ledger: the learning loop — winners bias future builds ----
  const [perf, setPerf] = useState([]);
  const [perfLoaded, setPerfLoaded] = useState(false);
  const [lastBuild, setLastBuild] = useState(null);
  useEffect(() => {
    storage.get("sablepress:perf").then((v) => {
      if (v && v.value) { try { setPerf(JSON.parse(v.value)); } catch (e) { /* fresh */ } }
      setPerfLoaded(true);
    }).catch(() => setPerfLoaded(true));
  }, []);
  useEffect(() => { if (perfLoaded) storage.set("sablepress:perf", JSON.stringify(perf)).catch(() => {}); }, [perf, perfLoaded]);

  function perfScores() {
    const sIdx = {}, qMap = {};
    perf.forEach((r) => {
      const sc = (r.views || 0) + 10 * (r.likes || 0) + 20 * (r.saves || 0) + 15 * (r.comments || 0);
      (r.stagings || []).forEach((i) => { sIdx[i] = (sIdx[i] || 0) + sc; });
      if (r.quote) qMap[r.quote] = (qMap[r.quote] || 0) + sc;
    });
    return { sIdx, qMap };
  }
  // weighted sampling without replacement: no data → plain seeded shuffle; with data → winners up to 4× likelier
  function weightedSample(items, scoreOf, k, rnd) {
    const scores = items.map((it) => scoreOf(it) || 0);
    const max = Math.max(...scores, 0);
    const pool = items.map((it, i) => ({ it, w: max > 0 ? 1 + 3 * (scores[i] / max) : 1 }));
    const out = [];
    while (out.length < k && pool.length) {
      const total = pool.reduce((s, p) => s + p.w, 0);
      let r = rnd() * total, j = 0;
      while (j < pool.length - 1 && r > pool[j].w) { r -= pool[j].w; j++; }
      out.push(pool.splice(j, 1)[0].it);
    }
    return out;
  }
  function perfLogLast() {
    if (!lastBuild) { alert("Build a campaign or flipcard set first — then log it after posting."); return; }
    if (perf.some((r) => r.postId === ringPostId)) { alert("Already logged: " + ringPostId); return; }
    setPerf([{ postId: ringPostId, date: new Date().toISOString().slice(0, 10), fmt: lastBuild.fmt,
      quote: lastBuild.quote, stagings: lastBuild.stagings, views: 0, likes: 0, saves: 0, comments: 0 }, ...perf]);
  }
  function perfSet(id, field, v) {
    setPerf(perf.map((r) => r.postId === id ? { ...r, [field]: Math.max(0, parseInt(v || 0, 10) || 0) } : r));
  }
  function perfRemove(id) { setPerf(perf.filter((r) => r.postId !== id)); }
  function perfCsv() {
    robustCopy("post_id,date,format,views,likes,saves,comments,quote\n" + perf.map((r) =>
      [r.postId, r.date, r.fmt, r.views, r.likes, r.saves, r.comments, `"${(r.quote || "").replace(/"/g, "'")}"`].join(",")).join("\n"),
      "Performance ledger copied as CSV.");
  }
  const perfTop = (() => {
    const { sIdx, qMap } = perfScores();
    const ts = Object.entries(sIdx).sort((a, b) => b[1] - a[1])[0];
    const tq = Object.entries(qMap).sort((a, b) => b[1] - a[1])[0];
    return {
      staging: ts && ts[1] > 0 ? `staging #${+ts[0] + 1}` : "—",
      quote: tq && tq[1] > 0 ? `“${tq[0].slice(0, 44)}${tq[0].length > 44 ? "…" : ""}”` : "—",
    };
  })();

  // ---- ☀️ daily kit: the whole 8AM/9PM slot in one paste ----
  const [dailyKit, setDailyKit] = useState("");
  function buildDailyKit() {
    const d = new Date();
    const dseed = d.getFullYear() * 10000 + (d.getMonth() + 1) * 100 + d.getDate();
    const rndK = mulberry32(dseed * 7 + (ringSlot === "AM" ? 1 : 2));
    const formats = ["Campaign set (3 scenes + quote card → slideshow video)", "8-flipcard set (→ flipbook)", "Firecard (procedural fire quote)", "Motion card (Ken Burns)"];
    const fmtPick = formats[Math.floor(rndK() * formats.length)];
    const pool = [...favLines, ...library.flatMap((b) => b.quotes || [])];
    const q = pool.length ? pool[Math.floor(rndK() * pool.length)] : offlineQuotes(3, dseed)[0];
    const baitLine = BAIT_LINES[Math.floor(rndK() * BAIT_LINES.length)];
    const seriesTag = (attribution.split(",")[0] || "").trim().toLowerCase().replace(/\W/g, "");
    const pin = PIN_QUESTIONS[dayOfYear % PIN_QUESTIONS.length];
    const cmts = COMMENT_BANK.slice(0, 3).map((b, i) => b.lines[(dayOfYear + i) % b.lines.length]);
    const kit = `☀️ DAILY KIT — ${d.toISOString().slice(0, 10)} · ${ringSlot} slot
FORMAT TODAY: ${fmtPick}
QUOTE: “${q}”
...
${KIT_TAGS[platform](seriesTag)} #slowburn
...
— RING —
Bio link: ${ringLink}
End-card: ${ringCta.endCard.replace(/\n/g, " ")}

— COMMENT ROUND (3 today, spread out, fresh videos <6h, never a link) —
1. ${cmts[0]}
2. ${cmts[1]}
3. ${cmts[2]}

Ritual: post → Log this post (Ring + Performance) → paste bio link.`;
    setDailyKit(kit);
    robustCopy(kit, "Daily kit copied — the whole slot in one paste.");
  }

  // ---- 💬 reply-to-comment cards: TikTok's highest-converting native format ----
  const [replyText, setReplyText] = useState("");
  const [replyLast, setReplyLast] = useState("");
  const replyLines = replyText.split("\n").map((s) => s.trim()).filter((s) => s.length > 3).slice(0, 12);
  function buildReplyCard(line) {
    const rndR = mulberry32(line.length * 131 + dayOfYear);
    const opener = REPLY_OPENERS[Math.floor(rndR() * REPLY_OPENERS.length)];
    const pool = [...favLines, ...library.flatMap((b) => b.quotes || [])];
    const q = pool.length ? pool[Math.floor(rndR() * pool.length)] : offlineQuotes(3, line.length * 977)[0];
    const c = canvasRef.current, ctx = c.getContext("2d");
    drawCard(ctx, fmt.w, fmt.h, pal, line.length * 53 + 9, { hook: `You asked: “${line}”`, quote: q, attribution: opener, handle });
    setGallery([{ src: c.toDataURL("image/jpeg", 0.92), label: "Reply card" }, ...gallery]);
    drawCard(ctx, fmt.w, fmt.h, pal, seed, { hook, quote, attribution, handle });
    const seriesTag = (attribution.split(",")[0] || "").trim().toLowerCase().replace(/\W/g, "");
    const cap = `Replying to the comment that wouldn't let me go.

${ringCta.caption}

${KIT_TAGS[platform](seriesTag)} #slowburn`;
    setReplyLast(cap);
    robustCopy(cap, "Reply card in gallery — caption copied. Post it via TikTok's “reply with video” on the original comment.");
  }

  // ---- ✉️ ring nurture: welcome sequence + weekly ember, in Lenore's voice ----
  const [nurtureOut, setNurtureOut] = useState("");
  function buildNurture(kind) {
    const series = (attribution.split(",")[0] || "").trim() || "the —BORN Universe";
    const pool = [...favLines, ...library.flatMap((b) => b.quotes || [])];
    const rndN = mulberry32(dayOfYear * 31 + kind.length * 7);
    const q = pool.length ? pool[Math.floor(rndN() * pool.length)] : offlineQuotes(3, dayOfYear * 17)[0];
    const T = {
      welcome1: `SUBJECT: The door closed behind you. Good.

You found the Inner Ring. Most readers scroll past the bio link; you didn't, and that tells me what kind of reader you are.

Here is what I promised — the scene that never made the book. It's attached below this letter, and it stays here. The Ring keeps what the books can't.

“${q}”

One email a week, at most. A line, a piece of the canon, sometimes a vote on what I write next. No noise.

— Lenore`,
      welcome2: `SUBJECT: Start where the gift is first held

If you came in through a quote, you're holding a thread with no beginning. The beginning is ${series === "GIFTBORN" ? "The Gift Held — Book One" : "Book One"}, and it's on Kindle Unlimited, which means if you already have KU, it costs you nothing but the night you'll lose to it.

The order, so you never have to ask: Held. Poured. Spent. Kept. Woven.

“${q}”

Read the first chapter. If it doesn't take you, delete this email and no hard feelings. It will take you.

— Lenore`,
      welcome3: `SUBJECT: Which line broke you?

A small ask. Reply to this email with the one line — from any of the books, or any of the videos — that stayed with you longest.

I read every reply. The lines you choose decide which scenes get deleted-scene treatment in the Ring, and more than once a reply here has changed what I wrote next.

“${q}”

That one still costs me something. Your turn.

— Lenore`,
      ember: `SUBJECT: One ember, as promised

This week's line:

“${q}”

Behind the canon: every gift in this universe is a ledger entry — something held is something owed, and the books are the accounting. That rule was decided before a single chapter existed, and it is why the endings land the way they do.

The Ring reads first, votes first, and keeps the deleted scenes. If someone forwarded you this: the door is at lenoresable.com/ring.

— Lenore`,
    };
    setNurtureOut(T[kind]);
    robustCopy(T[kind], "Email copied — paste into MailerLite.");
  }

  // ---- 📚 binge ladder: read-through cards ----
  const [ladderId, setLadderId] = useState("START");
  function buildLadderCard() {
    const L = LADDER.find((l) => l.id === ladderId) || LADDER[0];
    const c = canvasRef.current, ctx = c.getContext("2d");
    drawCard(ctx, fmt.w, fmt.h, pal, dayOfYear * 7 + ladderId.length * 13, { hook: L.hook, quote: L.line, attribution: L.cta, handle });
    setGallery([{ src: c.toDataURL("image/jpeg", 0.92), label: `Ladder — ${L.id}` }, ...gallery]);
    drawCard(ctx, fmt.w, fmt.h, pal, seed, { hook, quote, attribution, handle });
    const cap = `${L.hook}
${L.line}
...
${KIT_TAGS[platform]("giftborn")} #slowburn #kubooks`;
    robustCopy(cap, `Ladder card (${L.id}) in gallery — caption copied.`);
  }
...
STRICT: no watermark, no signature, no modern objects${withText ? ", no text other than the two captions specified above" : ", no text or lettering of any kind"}.`;
  ...
  // ---- 8-flipcard set: 8 different scenes, 8 different quotes, one ChatGPT prompt ----
  function buildFlipcardSet() {
    const n = Math.max(1, Math.floor(flipNo) || 1);
    const rnd = mulberry32(n * 6689 + 31);
    const shuffled = (arr) => {
      const a = [...arr];
    ...
    // 8 distinct stagings, seeded, biased toward proven performers
    const { sIdx: perfS, qMap: perfQ } = perfScores();
    const sceneIdx = weightedSample(STAGINGS.map((_, i) => i), (i) => perfS[i] || 0, STAGINGS.length, rnd).slice(0, 8);
    const scenes = sceneIdx.map((i) => STAGINGS[i]);

    // 8 distinct quotes: starred/mined first (performance-biased), seeded composer tops up
    const pool = [...new Set([...favLines, ...library.flatMap((b) => b.quotes || [])])];
    let quotes = weightedSample(pool, (x) => perfQ[x] || 0, Math.min(8, pool.length), rnd);
    if (quotes.length < 8) {
      for (const q of offlineQuotes(16, n * 33301 + 5)) {
    ...
    const blocks = scenes.map((st, i) => `============ IMAGE ${i + 1} of 8 — FLIPCARD ============

Create a 2D image: a traditional oil painting on textured canvas with visible brushstrokes, in a muted, painterly historical-fantasy style. Palette: ${mood}.

SCENE: ${st}.

CHARACTERS: On the left, ${charHer}. On the right, ${charHim}. Restrained body language, unspoken tension — longing without touching.

TEXT — RENDER EXACTLY, THIS IS CRITICAL:
One quote is painted directly onto the image as clean typeset text (not hand-lettered), elegant italic serif, soft off-white (#F5F0E6) with a subtle dark outer glow so it reads against sky and shadow. Exact text, verbatim, correct spelling, nothing added:
“${clean(quotes[i])}”
Position: centered horizontally, first line starting about 10% from the top, wrapped to 2–4 centered lines, block no wider than 78% of the image width. Each line's capital height about 3.2% of the image height. Keep the area behind the text simple (sky, mist, stone, shadow). No other text anywhere.

COMPOSITION: portrait orientation, 2:3 aspect ratio (e.g. 1024×1536). Full or three-quarter figures. Soft cinematic light, melancholic romantic atmosphere, fine grain and canvas texture.

STRICT: no watermark, no signature, no modern objects, no text other than the single quote specified above. Double-check the quote letter-for-letter before finishing.`);

    const prompt = `You will create EIGHT separate 2D images for me — a flipcard set. CRITICAL RULES:
- ONE image at a time, each as its OWN full-resolution image generation.
- NEVER combine them into a grid, collage, panel layout, or contact sheet.
- Generate ONLY IMAGE 1 now. Then WAIT. Each time I reply "next", generate the following image.
- Keep the SAME two characters, the same faces and clothing, and the same oil-painting style across ALL 8 images. Only the scene and the quote change.

${blocks.join("\n\n")}`;

    setFlipPrompt(prompt);
    robustCopy(prompt, "8-flipcard prompt copied — paste it into ChatGPT, reply \"next\" after each image.");
    setFlipInfo(`Flipcard set ${n} · ${quotes.length} quotes (${Math.min(pool.length, 8)} from starred/mined) · scenes seeded — same number always rebuilds the same set (until performance data shifts the bias).`);
    setLastBuild({ fmt: "flipcard", quote: quotes[0], stagings: sceneIdx });
    setFlipNo(n + 1);
  }
  ...
  // ---- Firecard: signature opener — record / play / still ----
  const fireFrame = (fs) => (ctx, el) => {
    const t = el / 1000;
    drawFireCard(ctx, fmt.w, fmt.h, pal, fs, { hook, quote, attribution, handle }, t, Math.min(1, el / 1200), fireVariant);
  };
  // Pick the seed for THIS export: reseed on = new pixels every time (fresh
  // fingerprint per post); reseed off = pin the current look.
  function nextFireSeed() {
    const s = fireReseed ? Math.floor(Math.random() * 100000) : fireSeed;
    if (s !== fireSeed) setFireSeed(s);
    return s;
  }
  function recordFireVideo() {
    recordVideoWith(fireFrame(nextFireSeed()), fireSecs * 1000);
  }
  function playFireForScreenRecording() {
    const c = canvasRef.current, ctx = c.getContext("2d");
    playWithCountdown(ctx, fireSecs * 1000, fireFrame(nextFireSeed()));
  }
  function saveFireStill() {
    const s = nextFireSeed();
    const c = canvasRef.current, ctx = c.getContext("2d");
    drawFireCard(ctx, fmt.w, fmt.h, pal, s, { hook, quote, attribution, handle }, 0.8, 1, fireVariant);
    setGallery([{ src: c.toDataURL("image/jpeg", 0.92), label: `Firecard · ${fireVariant} · seed ${s}` }]);
    drawCard(ctx, fmt.w, fmt.h, pal, seed, { hook, quote, attribution, handle });
  }
  // Motion lead-in: the Firecard opens the sequence, dips to black, then the
  // stills begin — reads as a title card, not a hard cut.
  function motionWithFire(fs) {
    const lead = fireSecs * 1000;
    return (ctx, el) => {
      if (el < lead) {
        const t = el / 1000;
        drawFireCard(ctx, fmt.w, fmt.h, pal, fs, { hook, quote, attribution, handle }, t, Math.min(1, el / 1200), fireVariant);
        if (el > lead - 600) {
          ctx.fillStyle = "#000";
          ctx.globalAlpha = (el - (lead - 600)) / 600;
          ctx.fillRect(0, 0, fmt.w, fmt.h);
          ctx.globalAlpha = 1;
        }
      } else motionSeqFrame(ctx, el - lead);
    };
  }
  function recordMotionVideo() {
    ...
    if (fireLead) {
      const fs = nextFireSeed();
      const lead = fireSecs;
      const cues = [0.2, lead];
      for (let i = 1; i < motionImgs.length; i++) cues.push(lead + i * motionPer);
      cues.push(lead + motionDurMs() / 1000 - 0.4);
      recordVideoWith(motionWithFire(fs), lead * 1000 + motionDurMs(), cues);
      return;
    }
  ...
  function playMotionForScreenRecording() {
    ...
    const c = canvasRef.current, ctx = c.getContext("2d");
    if (fireLead) {
      playWithCountdown(ctx, fireSecs * 1000 + motionDurMs(), motionWithFire(nextFireSeed()));
      return;
    }
  ...
  return (
    <div style={S.app}>
      ...
      <header style={S.header}>
        ...
        <span style={S.sub}>2D generator · cards · backdrops · video · motion · sound · firecard · v15</span>
      </header>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <div style={S.panel}>
              ...
              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                <div style={S.panelTitle}>🚀 POSTING — ONE TAP, READY TO PASTE</div>
                <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                  <button style={{ ...S.primaryBtn, flex: 1, padding: "18px 12px", fontSize: 15, lineHeight: 1.35 }} onClick={tapDescription}>
                    📝 Description<br /><span style={{ fontWeight: 500, fontSize: 12 }}>+ hashtags</span>
                  </button>
                  <button style={{ ...S.primaryBtn, flex: 1, padding: "18px 12px", fontSize: 15, lineHeight: 1.35, background: "#8C6A3F", color: "#EDE7DE" }} onClick={tapPinnedComment}>
                    📌 Pinned comment<br /><span style={{ fontWeight: 500, fontSize: 12 }}>once live</span>
                  </button>
                </div>
                {postLiveDesc && (
                  <textarea readOnly style={{ ...S.input, resize: "vertical", fontSize: 13, marginTop: 10 }} rows={8} value={postLiveDesc} onFocus={(e) => e.target.select()} />
                )}
                {postLivePin && (
                  <textarea readOnly style={{ ...S.input, resize: "vertical", fontSize: 13, marginTop: 10 }} rows={4} value={postLivePin} onFocus={(e) => e.target.select()} />
                )}
                <div style={{ marginTop: 12 }}>
                  <button style={{ ...S.primaryBtn, width: "100%", padding: "18px 12px", fontSize: 15, lineHeight: 1.35, background: "#5A7A6A", color: "#EDE7DE" }}
                    onClick={() => window.open("https://ads.tiktok.com/business/creativecenter/inspiration/popular/music/pc/en", "_blank")}>
                    🎵 Find trending sound<br /><span style={{ fontWeight: 500, fontSize: 12 }}>TikTok charts — pick, then search its name in the editor</span>
                  </button>
                  <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap" }}>
                    {["booktok sound", "dark romance audio", "romantasy edit audio", "dark aesthetic sound"].map((t) => (
                      <button key={t} style={{ ...S.ghostBtn, padding: "6px 10px", fontSize: 11.5 }}
                        onClick={() => window.open(`https://www.tiktok.com/search/video?q=${encodeURIComponent(t)}`, "_blank")}>🔍 {t}</button>
                    ))}
                  </div>
                  <p style={S.small}>Rota: trending audio on the big clips, Sablesound on brand/countdown clips. In TikTok: tap a result → tap the spinning disc → use sound, or add it by name in the editor.</p>
                </div>
              </div>

              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                <div style={S.panelTitle}>💬 DAILY COMMENT ROUNDS — 88/100</div>
                <p style={S.small}>3–5/day, spread out · fresh videos (&lt;6h) from bigger accounts · never a link, never a pitch. Tap 🔍 to open the TikTok search (then filter → most recent), tap the big button to copy today's line. Tap again for the next line.</p>
                {COMMENT_BANK.map((b, ci) => (
                  <div key={b.cat} style={{ marginTop: 10 }}>
                    <button style={{ ...S.primaryBtn, width: "100%", padding: "14px 12px", fontSize: 14, textAlign: "left" }} onClick={() => tapComment(ci)}>
                      {b.emoji} Copy comment — {b.cat}
                    </button>
                    <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap" }}>
                      {b.searches.map((t) => (
                        <button key={t} style={{ ...S.ghostBtn, padding: "6px 10px", fontSize: 11.5 }} onClick={() => openSearch(t)}>🔍 {t}</button>
                      ))}
                    </div>
                  </div>
                ))}
                {cmtLast && (
                  <textarea readOnly style={{ ...S.input, resize: "vertical", fontSize: 13, marginTop: 10 }} rows={3} value={cmtLast} onFocus={(e) => e.target.select()} />
                )}
              </div>

              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                <div style={S.panelTitle}>🔔 INNER RING — CTA + SIGNUPS PER POST</div>
                <p style={S.small}>Every post pushes to the Ring. Ritual: post → Log this post → copy bio link → paste as the TikTok bio link (5 seconds). Later, count subscribers per post_id in MailerLite and backfill the ledger. The caption tail is already baked into 📱 Description once live.</p>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-end" }}>
                  <div style={{ flex: 1, minWidth: 140 }}>
                    <label style={S.label}>Book</label>
                    <select style={S.input} value={ringBook} onChange={(e) => setRingBook(e.target.value)}>
                      {RING_BOOKS.map(([c, l]) => <option key={c} value={c}>{c} — {l}</option>)}
                    </select>
                  </div>
                  <div style={{ width: 130 }}>
                    <label style={S.label}>Slot</label>
                    <select style={S.input} value={ringSlot} onChange={(e) => setRingSlot(e.target.value)}>
                      <option value="AM">8AM PHT</option>
                      <option value="PM">9PM PHT</option>
                    </select>
                  </div>
                  <button style={{ ...S.ghostBtn, padding: "10px 14px" }} onClick={() => setRingTaps(ringTaps + 1)}>↻ next CTA</button>
                </div>
                <p style={{ ...S.small, color: "#E8A96B" }}>{ringPostId} · {ringCta.id} — “{ringCta.caption}”</p>
                <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                  <button style={{ ...S.primaryBtn, flex: 1, minWidth: 150 }} onClick={() => ringCopy("link")}>🔗 Copy bio link</button>
                  <button style={{ ...S.ghostBtn, flex: 1, minWidth: 130 }} onClick={() => ringCopy("caption")}>✍️ Caption tail</button>
                  <button style={{ ...S.ghostBtn, flex: 1, minWidth: 130 }} onClick={() => ringCopy("pinned")}>📌 Ring pin</button>
                  <button style={{ ...S.ghostBtn, flex: 1, minWidth: 130 }} onClick={() => ringCopy("endcard")}>🖼 End-card text</button>
                  <button style={{ ...S.primaryBtn, flex: 1, minWidth: 150, background: "#8C6A3F", color: "#EDE7DE" }} onClick={ringLog}>＋ Log this post</button>
                </div>
                {ringLast && (
                  <textarea readOnly style={{ ...S.input, resize: "vertical", fontSize: 13, marginTop: 10 }} rows={2} value={ringLast} onFocus={(e) => e.target.select()} />
                )}
                {ringLedger.length > 0 && (
                  <div style={{ marginTop: 12 }}>
                    <p style={{ ...S.small, marginTop: 0 }}>
                      <b style={{ color: "#E8A96B" }}>{ringTotal}</b> signups · {ringLedger.length} posts · AM {ringAM} / PM {ringPM} · best CTA: <b style={{ color: "#E8A96B" }}>{ringBest}</b>
                    </p>
                    {ringLedger.map((r) => (
                      <div key={r.postId} style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 6 }}>
                        <span style={{ fontSize: 12, color: "#B0A69A", flex: 1, fontFamily: "'IBM Plex Mono', monospace" }}>{r.postId} · {r.ctaId}</span>
                        <input type="number" min={0} style={{ ...S.input, width: 74, display: "inline-block", padding: "6px 8px", fontSize: 13 }}
                          value={r.signups} onChange={(e) => ringSetSignups(r.postId, e.target.value)} />
                        <button style={{ ...S.ghostBtn, padding: "6px 10px", fontSize: 11.5 }} onClick={() => ringRemove(r.postId)}>✕</button>
                      </div>
                    ))}
                    <button style={{ ...S.ghostBtn, padding: "8px 12px", fontSize: 12, marginTop: 10 }} onClick={ringCsv}>📄 Copy ledger as CSV</button>
                  </div>
                )}
              </div>

              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                <div style={S.panelTitle}>☀️ DAILY KIT — THE WHOLE SLOT IN ONE TAP</div>
                <p style={S.small}>One tap = today's complete post package for the current slot: recommended format (rotates by date), quote, caption with bait line + Ring tail + hashtags, pinned comment, bio link, and today's 3 comment-round lines. Seeded by date — the 8AM and 9PM kits differ.</p>
                <button style={{ ...S.primaryBtn, width: "100%", marginTop: 8 }} onClick={buildDailyKit}>☀️ Build & copy today's kit ({ringSlot})</button>
                {dailyKit && (
                  <textarea readOnly style={{ ...S.input, resize: "vertical", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5, marginTop: 8 }} rows={10} value={dailyKit} onFocus={(e) => e.target.select()} />
                )}
              </div>

              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                <div style={S.panelTitle}>📈 PERFORMANCE — THE LEARNING LOOP</div>
                <p style={S.small}>After posting a campaign or flipcard set, tap Log. Later, backfill views / likes / saves / comments from TikTok analytics. Every builder then biases its seeded picks toward winning stagings and quotes — winners become up to 4× likelier. Score = views + 10×likes + 20×saves + 15×comments.</p>
                <button style={{ ...S.primaryBtn, width: "100%", marginTop: 8, background: "#8C6A3F", color: "#EDE7DE" }} onClick={perfLogLast}>＋ Log last build ({lastBuild ? lastBuild.fmt : "none yet"}) as {ringPostId}</button>
                {perf.length > 0 && (
                  <div style={{ marginTop: 10 }}>
                    <p style={{ ...S.small, marginTop: 0 }}>Top performer so far: <b style={{ color: "#E8A96B" }}>{perfTop.staging}</b> · {perfTop.quote}</p>
                    {perf.map((r) => (
                      <div key={r.postId} style={{ marginTop: 8, padding: "8px 10px", border: "1px solid #3A342E", borderRadius: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 11.5, color: "#B0A69A", flex: 1, fontFamily: "'IBM Plex Mono', monospace" }}>{r.postId} · {r.fmt}</span>
                          <button style={{ ...S.ghostBtn, padding: "4px 8px", fontSize: 11 }} onClick={() => perfRemove(r.postId)}>✕</button>
                        </div>
                        <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                          {["views", "likes", "saves", "comments"].map((f) => (
                            <div key={f} style={{ flex: 1 }}>
                              <label style={{ ...S.label, fontSize: 10 }}>{f}</label>
                              <input type="number" min={0} style={{ ...S.input, padding: "6px 6px", fontSize: 12 }} value={r[f]} onChange={(e) => perfSet(r.postId, f, e.target.value)} />
                            </div>
                          ))}
                        </div>
                      </div>
                    ))}
                    <button style={{ ...S.ghostBtn, padding: "8px 12px", fontSize: 12, marginTop: 10 }} onClick={perfCsv}>📄 Copy performance CSV</button>
                  </div>
                )}
              </div>

              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                <div style={S.panelTitle}>📚 BINGE LADDER — READ-THROUGH CARDS</div>
                <p style={S.small}>Quotes sell mood; KU pays on read-through. Pick a transition, tap: the card lands in the gallery and the caption is copied. START is the "begin at Book One" card for new viewers; GB→DEBT is the cross-universe hook.</p>
                <div style={{ display: "flex", gap: 8, alignItems: "flex-end", marginTop: 8 }}>
                  <div style={{ flex: 1 }}>
                    <label style={S.label}>Transition</label>
                    <select style={S.input} value={ladderId} onChange={(e) => setLadderId(e.target.value)}>
                      {LADDER.map((l) => <option key={l.id} value={l.id}>{l.id} — {l.hook}</option>)}
                    </select>
                  </div>
                  <button style={{ ...S.primaryBtn, minWidth: 150 }} onClick={buildLadderCard}>📚 Build card</button>
                </div>
              </div>

              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                <div style={S.panelTitle}>💬 REPLY-TO-COMMENT CARDS</div>
                <p style={S.small}>TikTok's highest-converting native format. Paste comments from your own videos (one per line), tap one: a reply card lands in the gallery and the caption is copied. Post it via TikTok's "reply with video" on the original comment — free social proof plus algorithm juice.</p>
                <textarea style={{ ...S.input, resize: "vertical", marginTop: 8 }} rows={4} placeholder={"is this from a real book??\nwhose line is the second one\nI need book 3 NOW"} value={replyText} onChange={(e) => setReplyText(e.target.value)} />
                {replyLines.length > 0 && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 8 }}>
                    {replyLines.map((ln, i) => (
                      <button key={i} style={{ ...S.ghostBtn, textAlign: "left", fontSize: 12.5 }} onClick={() => buildReplyCard(ln)}>💬 “{ln.slice(0, 70)}{ln.length > 70 ? "…" : ""}”</button>
                    ))}
                  </div>
                )}
                {replyLast && (
                  <textarea readOnly style={{ ...S.input, resize: "vertical", fontSize: 12, marginTop: 8 }} rows={3} value={replyLast} onFocus={(e) => e.target.select()} />
                )}
              </div>

              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                <div style={S.panelTitle}>✉️ RING NURTURE — WELCOME SEQUENCE + WEEKLY EMBER</div>
                <p style={S.small}>The list is the only owned asset — this keeps it warm. One tap per email, in Lenore's voice, filled from your starred/mined lines. Paste into MailerLite: welcome 1–3 as the automation, Ember as the weekly send. Uses the currently selected series/attribution.</p>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                  <button style={{ ...S.ghostBtn, flex: 1, minWidth: 130 }} onClick={() => buildNurture("welcome1")}>1 · The door</button>
                  <button style={{ ...S.ghostBtn, flex: 1, minWidth: 130 }} onClick={() => buildNurture("welcome2")}>2 · Start Book One</button>
                  <button style={{ ...S.ghostBtn, flex: 1, minWidth: 130 }} onClick={() => buildNurture("welcome3")}>3 · Reply ask</button>
                  <button style={{ ...S.primaryBtn, flex: 1, minWidth: 130 }} onClick={() => buildNurture("ember")}>🔥 Weekly ember</button>
                </div>
                {nurtureOut && (
                  <textarea readOnly style={{ ...S.input, resize: "vertical", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5, marginTop: 8 }} rows={10} value={nurtureOut} onFocus={(e) => e.target.select()} />
                )}
              </div>
              ...
              <div style={{ borderTop: "1px solid #2A2622", marginTop: 16, paddingTop: 12 }}>
                ...
                <button style={{ ...S.primaryBtn, width: "100%", marginTop: 8, background: "#6B4F7A", color: "#EDE7DE", opacity: recording ? 0.6 : 1 }} disabled={recording}
                  onClick={() => { if (vidImgs.length < 4) { alert("Flipbook needs at least 4 frames, in order."); return; } runFlipbookVideo(vidImgs); }}>
                  {recording ? "Working… watch the preview" : `🎞 Make flipbook (${vidImgs.length} frames, hard cuts, loops)`}
                </button>
                <p style={S.small}>Flipbook = frame-by-frame animation frames (fire rising, small movements). Select ONLY the animation frames in order — no quote card, no cover. 6fps, ping-pong loop ×3, holds the last frame. Add quote + cover cards afterwards in TikTok, or post the loop raw.</p>
                <p style={S.small}>The finished MP4 appears in the save section below with a save button.</p>

                <div style={{ borderTop: "1px solid #3A342E", marginTop: 14, paddingTop: 12 }}>
                  <div style={S.panelTitle}>🃏 8-FLIPCARD SET — ChatGPT PROMPT</div>
                  <p style={S.small}>One tap = one numbered set: a SINGLE ChatGPT prompt ordering 8 separate images — 8 different scenes, each with a different quote painted on it, same two characters and oil style throughout. Uses your starred/mined lines first. Same number always rebuilds the same set; the counter advances automatically. Save the 8 paintings, then run them through the flipbook or slideshow above.</p>
                  <div style={{ display: "flex", gap: 8, alignItems: "flex-end", marginTop: 8 }}>
                    <div style={{ width: 110 }}>
                      <label style={S.label}>Set #</label>
                      <input type="number" min={1} step={1} style={S.input} value={flipNo} onChange={(e) => setFlipNo(Math.max(1, Math.floor(+e.target.value || 1)))} />
                    </div>
                    <button style={{ ...S.primaryBtn, flex: 1 }} onClick={buildFlipcardSet}>🃏 Build & copy 8-flipcard prompt</button>
                  </div>
                  {flipInfo && <p style={S.small}>{flipInfo}</p>}
                  {flipPrompt && (
                    <div>
                      <button style={{ ...S.ghostBtn, width: "100%", marginTop: 8 }} onClick={() => robustCopy(flipPrompt)}>Copy the full 8-image ChatGPT prompt</button>
                      <textarea readOnly style={{ ...S.input, resize: "vertical", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5, marginTop: 8 }} rows={8} value={flipPrompt} onFocus={(e) => e.target.select()} />
                    </div>
                  )}
                </div>
              </div>
            ...
            <div style={S.panel}>
              <div style={S.panelTitle}>🖋 LENORE VOICE — UNIVERSE COPY + RING EXCLUSIVES</div>
              <p style={S.small}>Her voice on demand, any —BORN Age, current or future. Fiction modes write Ring-exclusive bonus scenes and spoiler-safe teasers; the rest writes captions, hooks, emails, blurbs, replies. Canon notes are honored exactly — where you give none, it stays atmospheric and invents no plot.</p>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>
                {LV_MODES.map((m) => (
                  <button key={m.id} style={{ ...S.ghostBtn, padding: "8px 12px", fontSize: 12, ...(lvMode === m.id ? { borderColor: "#E8A96B", color: "#E8A96B" } : {}) }}
                    onClick={() => { setLvMode(m.id); setLvOut(null); setLvErr(""); }}>
                    {m.name}
                  </button>
                ))}
              </div>
              <label style={S.label}>Series / Age</label>
              <select style={S.input} value={lvSeries} onChange={(e) => setLvSeries(e.target.value)}>
                {LV_SERIES.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
              <label style={S.label}>Context</label>
              <textarea style={{ ...S.input, resize: "vertical" }} rows={3} value={lvInput}
                placeholder={(LV_MODES.find((m) => m.id === lvMode) || {}).hint}
                onChange={(e) => setLvInput(e.target.value)} />
              {(LV_MODES.find((m) => m.id === lvMode) || {}).fiction && (
                <>
                  <label style={S.label}>Canon notes (optional — honored exactly)</label>
                  <textarea style={{ ...S.input, resize: "vertical", borderStyle: "dashed" }} rows={2} value={lvCanon}
                    placeholder="Character names, facts, or lines that must hold"
                    onChange={(e) => setLvCanon(e.target.value)} />
                </>
              )}
              <button style={{ ...S.primaryBtn, width: "100%", marginTop: 12, opacity: lvBusy || !lvInput.trim() ? 0.55 : 1 }}
                disabled={lvBusy || !lvInput.trim()} onClick={generateLenore}>
                {lvBusy ? "Tending the fire…" : "✒️ Write in her voice"}
              </button>
              {lvErr && <p style={{ ...S.small, color: "#D98B6B" }}>{lvErr}</p>}
              {lvOut && lvOut.map((v, i) => (
                <div key={i} style={{ border: "1px solid #2A2622", borderRadius: 8, padding: "10px 12px", marginTop: 10, background: "#221E1A" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ ...S.panelTitle, fontSize: 10.5 }}>{v.label}</span>
                    <button style={{ ...S.ghostBtn, padding: "5px 10px", fontSize: 11.5 }} onClick={() => robustCopy(v.text, "Copied.")}>Copy</button>
                  </div>
                  <div style={{ fontSize: 13.5, lineHeight: 1.55, whiteSpace: "pre-wrap", marginTop: 6, color: "#EDE7DE" }}>{v.text}</div>
                </div>
              ))}
            </div>
            ...
            <div style={S.panel}>
              <div style={S.panelTitle}>🔥 Firecard — signature opener, never the same pixels twice</div>
              <p style={S.small}>Your recurring opening card, built procedurally — no uploads. The layout (flame tongues, candle count and placement, ember field) comes from a seed, so followers see the same brand look while every export carries fresh pixels: no dedupe flag, no "seen it" swipe. Uses the current quote, palette and handle.</p>
              <div style={{ display: "flex", gap: 16, marginTop: 10, flexWrap: "wrap", alignItems: "center" }}>
                <div style={{ width: 190 }}>
                  <label style={S.label}>Scene</label>
                  <select style={S.input} value={fireVariant} onChange={(e) => setFireVariant(e.target.value)}>
                    <option>Hearth fire</option>
                    <option>Candlelight</option>
                    <option>Ember drift</option>
                    <option>Cold flame</option>
                  </select>
                </div>
                <div style={{ width: 110 }}>
                  <label style={S.label}>Seconds</label>
                  <input type="number" min={3} max={15} style={S.input} value={fireSecs} onChange={(e) => setFireSecs(Math.max(3, Math.min(15, +e.target.value || 6)))} />
                </div>
                <label style={{ ...S.small, display: "flex", alignItems: "center", gap: 6, margin: 0 }}>
                  <input type="checkbox" checked={fireReseed} onChange={(e) => setFireReseed(e.target.checked)} /> New scene every export
                </label>
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 8, alignItems: "center" }}>
                <button style={S.ghostBtn} onClick={() => setFireSeed(Math.floor(Math.random() * 100000))}>↻ Shuffle scene</button>
                <span style={S.small}>Seed {fireSeed} — reproducible</span>
              </div>
              <button style={{ ...S.primaryBtn, width: "100%", marginTop: 12, opacity: recording ? 0.6 : 1 }} onClick={recordFireVideo} disabled={recording}>
                {recording ? "Working… watch the preview" : `🎬 Record Firecard (${fireSecs}s · ${fireVariant})`}
              </button>
              <button style={{ ...S.ghostBtn, width: "100%", marginTop: 8, opacity: recording ? 0.6 : 1 }} onClick={playFireForScreenRecording} disabled={recording}>
                ▶ Play Firecard (for iOS screen recording)
              </button>
              <button style={{ ...S.ghostBtn, width: "100%", marginTop: 8 }} onClick={saveFireStill}>
                📸 Save still → gallery (carousel slide 1)
              </button>
              <label style={{ ...S.small, display: "flex", alignItems: "center", gap: 6, marginTop: 10 }}>
                <input type="checkbox" checked={fireLead} onChange={(e) => setFireLead(e.target.checked)} /> Open every Motion sequence with the Firecard ({fireSecs}s title card, dips to black, then your stills)
              </label>
              <p style={S.small}>Candlelight reads intimate; Hearth reads intense; Ember drift is quote-forward; Cold flame suits COLDBORN / TIDEBORN palettes. Same fonts and layout as your painted card — the quote is the only thing followers should notice changing.</p>
            </div>
